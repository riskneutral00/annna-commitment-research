...
**Consequence for scope, carried into §6:** rings one and two are Situation A shaped. `README.md` opens with the Situation C week because it is the strongest demonstration, and that is the right choice for a front page — but MVP scope is cut to the cohort, not to the demonstration. Cold start, listed as a live risk in `PR/BRIEF.md`, barely bites this cohort: Situation A delivers its whole promise with one person on the system. It bites on one seam only — a freelance instructor accepting *jobs* needs a center on annnä, and centers are ring three.
...
Two journeys have no story, and both are new with this document.
