...
- **Institutions running an autonomy envelope.** Not a v1 user; the held-out ER probe that explored the shape (Situation E) retired 2026-09-17, FD-108.
...
The audience is *"the mirror, not the market"*, one that *"grows in circles"* (the retired identity file's phrase, FD-108). The first two circles are now nameable, and naming them supplies the cohort the pilot item needed — that item is closed (OR-40 → FD-77, 2026-08-23): §10.1 records the founder's own English students as the named pilot.
...
- **Not a claim to have invented multi-resource scheduling.** The 2026-08-08 landscape scan (Motion, Reclaim, Calendly on one side; Anolla, Aquateks, Roverd, Bookeo, EVE Diving, Anchor on the other — recorded in the retired `PR/BRIEF.md`, FD-108) established that vertical booking software already schedules people and equipment together, and that no outward surface may imply otherwise. The claim is the unoccupied combination, and it is a narrowing.
...
*This section is the risk register's normative home since 2026-09-17 (FD-108; it was `PR/BRIEF.md` until that file retired). The first five are this document's own (the fifth added at Finalize, 2026-08-22; carried as surviving review under FD-101). The six that follow were the register's rows, re-homed here verbatim. Each entry is a live risk, not a resolved one.*
...
- **The agent-first claim can outrun its scenarios.** Every outward claim must be grounded in specified behavior, and the 2026-08-08 landscape scan set the precedent by *narrowing* a claim rather than widening it. Saying annnä works with an outside agent, before one has, is that same shape.
...
- **No user has used it.** All validation is internal: falsification probes and structured adversarial review by AI reviewers, not external human audits. That catches design errors. It does not catch wanting.
...
- **Legal.** annnä's role in the data its owners admit — the owner's business as controller and annnä as processor where that relationship applies (`security/SPEC.md §12`) — and eventually routing customers between businesses, are legal questions before engineering ones. A formal legal review is a hard build gate that green tests cannot argue past (`security/BUILD.md` Step 8); the referral beat is deferred behind it (FR1).
...
The success criteria are ruled (2026-08-08) and this section is their only home since 2026-09-17 (FD-108; it was `PR/BRIEF.md` until that file retired). The signals that count, in order:
...
Two secondary signals, admitted as hard to measure cleanly: time spent looking at their own commitments goes down; time spent in scheduling tools goes down against whatever they used before. **No numeric target is set, and none may be introduced by derivation.** A target becomes settable when the first pilots run.

**One readiness condition joins the criteria (FD-63, amended FD-79):** the founder's own real bookings run on the system with **zero fallbacks** to the old tools — binary, evaluated continuously from first live use over his real teaching weeks *including his students' bookings*; dogfood and pilot are the same run, not sequential. It is a readiness condition, not a schedule (FR2).

**The pilot cohort (OR-40, closed FD-77, 2026-08-23):** the founder's own English students, interacting as guest parties through tokenized links — never accounts. Recruitment proceeds when the deployed first-phase slice is green and the founder's own live teaching week runs on annnä with student bookings flowing through it. The cohort size is a business target and is deliberately not stated as a number here.
...
*Five of the questions this document opened were answered on 2026-08-08 and now appear as decisions where they belong: v1's scope (§6.1), what the legal review reviews (§6.3), where this document lives (§0.1), and the two adopted answers behind RQ-1 and RQ-8. **Q1 closed on 2026-08-09**, **Q3 on 2026-08-21**, and **Q2 on 2026-08-22**; all are kept below rather than deleted, because a reader who finds only the answer will re-ask the question. **The original Q1–Q3 are closed; item 4 still records open wants and routed questions. Their scope and readiness implications must be assessed rather than hidden by a blanket “none open.”***
...
4. **Open founder wants and routed product questions** *(re-homed 2026-09-17, FD-108, from the retired `app/NOTES.md` and `PR/NOTES.md`; none is ruled, none is designed, none blocks a build step)*:
   - **A routines page** (founder, 2026-08-23) — a separate view or filter of the daily/weekly patterns, toggled on and off, with edit affordances. Routines are already `engine/SPEC.md §1.4` patterns; the fork is a page (needs a `Pattern` read path specified engine-side) versus a board filter or a `routines` wake scope (display-only, cheap). The one open shape: a routine the owner wants recorded but never materialized.
...
**Phase 1 review, 2026-09-20.** Planning, development and live maintenance are distinct (`AGENTS.md` §Project phases). A scripted journey is a falsification prompt, not customer validation or a model result. The following assumptions need explicit evidence or an accepted bounded deferral before a development go-word; they add no capability or numerical business target.

| Assumption | Evidence needed / consequence if wrong | Home / disposition |
|---|---|---|
| The first owner receives useful value before counterparties adopt | Walk the accepted owner-first flow against current practice; record setup, corrections and coordination burden. Supplier uptake and silent placement are separate hypotheses. | RQ-14, §10.1; retain the English-student guest cohort, not a new vertical product. |
| Agent-first interaction makes common corrections easier | Define a comparison task and observable time-to-correct, ask-again and mistaken-acceptance outcomes; do not substitute agent-conformance scores for usability. | §10.1 and item 4 of §11; no numeric target inferred. |
| Cheap model/provider availability makes the workflow affordable and fast | Full-path measurement including retries, quarantine, reviews and failures; log version and unknown usage. | `model/INTERFACES.md` §2.2; optional local cost worksheet, never a pricing promise. |
| Clear narrative and valid schemas establish the correct intent | Paired absent-candidate, negation, scope, correction and compound-instruction cases; keep raw source evidence and engine verification. | `model/SPEC.md` §§0/3 and N/A/Z sets. |
| Current model and data-processing arrangements remain suitable | Qualification per binding/language plus a documented change/withdrawal response; no default switch to an unqualified model. | `model/EVALS.md`, `security/SPEC.md` §§5/12. |

**Planning decision rule:** resolve genuine product-scope forks in their homes; a user's preferences remain questions for that user at the appropriate scope. Keep optional research and future operations questions visible as deferred with an owner and trigger. Do not make every unknown, provider account, or future feature a Phase 1 blocker. Existing v1 scope, trial-cohort ruling and product success definition remain unchanged.
