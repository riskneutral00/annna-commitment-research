...
| RQ-11 | Situation-A, situation-5 — Brownie's booking arrives to attention over the muted rest state, then the board settles around it (same scripted probe) |
...
| RQ-14 | Situation-A — set down once, by talking: empty account to a kept board and a pasteable live link in one sitting, no second party (`story-sofia.md` + situation-1) |
...
- **The first release is Situation A plus the freelancer half of Situation C** — the founder's own two jobs, the only slice with a live first user. Stated as readiness, never schedule (FR2): the release exists when its slice's scenarios are green and the §6.4 conditions for its rings hold.
...
- **The owed mechanism:** every `[MUST]` scenario carries a release tag and untagged fails — landed as its own mechanical pass so the gate lands green, not red.
