...
- The **recorded firings** the exam replays — the third evidence artifact, and the one the other two are checked against. Their custody is `BUILD.md` Step 0's snapshot-custody text: replay by default and keyless, a separate deliberate keyed command to re-record, normalization to a fixed point, and secrets structurally absent. Cited here; stated once, there.
