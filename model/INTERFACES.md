...
- **This layer composes `messages`, and the composition is deterministic** *(2026-08-29 — `complete()` takes a `messages` array and no prompt argument, so somebody joins the prompt to the context, and neither side said which)*. **Who owns that join is the harness seam's to state, and it states it** — `../harness/INTERFACES.md §2`; what follows satisfies that statement instead of standing in for it *(2026-09-13 — this bullet used to close on a conditional aside disclosing that a derived file was the rule's only home, and the paired harness statement is what retires it)*. **The join is over the whole of each call's input, never the context alone** *(2026-09-13 — it was written `{prompt asset (§3), serialized context} → messages`, and a literal implementation of that two-input recipe composes identical bytes for "book me in" and "cancel it" whenever the context object matches, because the `utterance` is a separate argument at the seam above)*: `normalize` joins the prompt asset (§3) to the `utterance` **and** the serialized `context` object (`../harness/INTERFACES.md §2.1`'s `{standing_frame, board_summary, relevant_slice, handoff_frame?}`, which the harness assembles and hands across with no prompt) · `narrate` joins the asset to the serialized `structure` · `summarize` joins the asset to the `raw_text` **and** its `source_tag`. **Replay identity is that whole input together with the binding that ran it** — the full call input, and beside it the qualified `provider`, `model_id` and prompt hash the call resolved to (§2.2, §3): the same inputs under the same binding produce the same bytes every time, which is what makes a recorded firing replayable at all, and a changed `utterance`, `structure`, `raw_text`, `source_tag` or binding is a **different** firing rather than the same one replayed. It **satisfies** the wire-order law rather than restating it — stable material first, volatile slice after, so provider prompt caching gets a stable prefix; that law's home is `../harness/SPEC.md §8` ("the serialized prompt puts the **stable material first**"). **Nothing else enters `messages`.** In particular **no tool schemas ride**, and the model never selects a tool call: the seam carries exactly four call types and owns that fact at `../harness/INTERFACES.md §2` — cited, not re-ruled here.
- **Replay identity includes the complete qualified binding.** The binding beside the whole call input includes `provider`, `model_id`, `temperature`, optional `reasoning_effort`, and the prompt hash. Changing only `temperature` or `reasoning_effort` therefore changes the replay identity and misses a recording; the provider-contract suite must assert this sampling-only counterexample alongside the call-input mutations.
