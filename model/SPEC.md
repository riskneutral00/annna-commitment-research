...
The harness consumes these (contract shapes in `../harness/INTERFACES.md §2`); this layer must *implement* them — and it also composes what goes on the wire, the prompt asset joined to the serialized context (`INTERFACES.md §2.1`), since the harness hands over a context object and no prompt:
...
**A cost-cap trip is a failure of that call, and takes the ladder above** *(drafted by an agent and not yet ratified — treat the reasoning as a proposal until the founder registers it; the rule itself is stated here because this is where a builder looks for it)*. When a call would exceed its binding's `max_cost_per_call` (`INTERFACES.md §2.2`), the call **fails** and enters rung 1 — never a silent truncation, never a degraded partial, never an untracked overrun. **The spend still counts** toward the firing's `firing_budget` and toward the per-credential daily model-spend cap (`../security/SPEC.md §10`): a cap trip escapes neither ceiling, or the cap would be a way to spend money that no ceiling sees. Every sibling ceiling in the corpus states its outcome — the firing budget parks, the daily cap suspends and raises one console card — and this one now does too. Naming the cause when such a trip **parks** an unattended firing is not this layer's to do: park reasons are a closed vocabulary at their own home (`../harness/SPEC.md §3.4`), and the model layer only fails the call.
