...
- **`normalize(utterance | trigger_event, context) → {intent, fields, ambiguities}`** — the first argument is the owner's utterance on a console turn, or the **trigger event** on a trigger firing *(the union stated 2026-08-21 — trigger firings have no utterance, and inventing a synthetic one would owner-tag fabricated text; the seam stays at four calls)*. Obligations: return an intent from the vocabulary in §2 (never a free-form action name); return field values **raw and unvalidated** (they are not trusted — the harness routes correctness-critical ones through the engine); populate `ambiguities` per the calibration in §3; use *only* the assembled context (never remembered conversation — there is none: the harness assembles context fresh per firing).
