...
**And an admitted growth item is held out by default** *(2026-08-29)*. Every seed item above is published in this file, and nothing stopped a prompt author from teaching to them — **a bar you can memorize is not a floor**, which is precisely what the 100% bars claim to be. So: a growth item admitted from production is **held out** unless it is deliberately published; **a held-out item never appears in prompt text or in a few-shot example**; and an item that **leaks into a prompt is retired from the bar** — it can stay as a regression check, but it stops counting toward a threshold, because it no longer measures generalization. The vocabulary is not new: `../harness/SCENARIOS.md` already tags scenarios `[HELD-OUT]` for the same reason.
...
**Coverage condition** *(2026-08-29)*: **every member of `labels[]`'s closed enum (`INTERFACES.md §2.4`) carries at least one S-set item before the `summarize` binding qualifies** — **`unresolved-reference` explicitly included**, since `SPEC.md §5` makes it a rule ("an unresolved reference stays unresolved and is reported as such") and nothing below grades it, while the carry-through bar covers the `labels[]` choice. A label the schema forces and nothing grades is a field the model is free to be wrong about.
...
FD-16's curve is about *repeated* success, and every set above grades single attempts. The P-set is the instrument for the other half, and it is deliberately cheap: **grading is an engine-state assertion, not a judgment about text** — after the turn runs, stored state either matches the expected commitment or it does not, code-graded, no judge. Cheap matters because of the run count the repetition rule below sets: **this is the most expensive set in the exam**, and it re-runs on every candidate and on every prompt version (§3 step 1). *(Re-justified 2026-08-29: the reason this line used to give was a per-provider re-run forced by FR5's BYO key — which §3 forbids twice over, "An owner's own model (SPEC §7) never runs this exam" and "No BYO binding may ever be recorded as qualified." It was a survivor of pre-confinement thinking; the run count is the real reason, and it is stated once, below.)*
...
Per-language mirrors of N-01/N-03/A-01/A-02, **R-02/R-04**, **and S-01/S-02 — the S-mirrors** *(2026-08-31, F-13: the quarantine read's two modes graded per language, `SPEC.md §6`)* (same expected outputs; utterances, read-backs and stranger text in the target language), one sub-set per required language — currently **th, zh-TW, en** per SPEC §6 (derived from the user-stories corpus: Hug Ocean zh/th/en, Thai resource owners). A model failing any required language's sub-set is **disqualified for that call type outright**, regardless of price — the consequence and its slot scope are SPEC §6's rule, cited here rather than restated.
...
1. Any change to **model, prompt, or routing config** → run the full exam for the affected call types. **The J-set and the Q-set attach to a carrying binding, not to a call type of their own** *(2026-08-29 — judgment is not separately bound, `INTERFACES.md §2.2`, so neither set had a re-run trigger while both hold a 100% bar)*: a change to the **`normalize` or `narrate`** binding — model, prompt, sampling value, or any other config value it is qualified at — re-runs **J and Q for that binding**, because judgment and injection resistance ride inside those two privileged calls and their failures are that binding's failures.
2. Pass = every set meets its threshold, **and** the 100%-bars (§1's canonical enumeration) are perfect.
3. Record `{model_id, prompt version, set scores, date, **cost per call type, p95 latency**, **recorded-firings reference**}` in the routing config's qualification state *(cost and latency added 2026-08-22 — the exam graded correctness while "build with frontier, run with cheap" is a cost-and-latency decision, so the two numbers the swap decision runs on were the two the record omitted; recorded, not thresholded — the threshold is the ops decision's, this record is its evidence)*; a binding without a current record does not go live. **A `fallback_model_id` entry is a binding** *(2026-08-29)*: it carries its own current full-exam record, and an unqualified fallback refuses to load exactly as an unqualified primary does. The fallback is the model that actually runs, unattended, when the primary times out — which is precisely where SPEC §7's law bites: *an unqualified model may never act while nobody is watching.* The obligation holds for all three separately-bound call types, not only for `summarize`, whose app-supplied pin (`INTERFACES.md §2.2`) is a supply rule and never was a qualification one. **A record whose firings cannot replay keyless is not a current record** *(the recorded-firings reference added 2026-08-29)*: a score nobody can reproduce is a number, not evidence, so the record names the artifact that reproduces it. What a recording is, and how it is made and kept, is `BUILD.md` Step 0's snapshot custody, part of **FD-84** (Drafted 2026-08-28, not ratified — pending FD-84) as Step 0 itself carries it — cited here, stated only there. **Only a full-exam record is a record**: a partial run — one set, one metric, the DR-7 spike's N-only number included — is never a qualification record *(the rule two files already cited this section for, now stated by it — 2026-08-21)*. **The record includes every value the binding was qualified at — `INTERFACES.md §2.2`'s qualified-at fields, picked up by citing that list rather than copying it here** *(2026-08-29: the list was a single field when it was written and has grown since; a second copy of it is a survivor waiting to happen)*: a swap to a smaller `context_budget_tokens`, or a change to a sampling value (`temperature`, `reasoning_effort` — the field law is §2.2's, not restated here), is a re-qualification, not a config diff — and **the exam runs at the budget it qualifies** *(2026-08-22 — the items are short, so a passing score previously said nothing about the one number it records)*: each item's assembled context is **padded to the binding's declared `context_budget_tokens`** with realistic slice material (rules and stored answers in the fixture corpus's shape, never adversarial text — the Q/S sets own that), so the score measures the model at the budget it will serve, not at toy length.
