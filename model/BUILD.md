...
- Seed with the EVALS §2 items as they stand — **including the S-set**, whose two modes (carry-through, loss) are scored separately from the start; a scaffold that reports one S number has already lost the loss trap.
...
- **Verify:** at least one binding per call type passes; the 100%-bars (R-invention, J-forbidden) are perfect; every required language's Z sub-set (th, zh-TW, en — SPEC §6) passed by the `normalize` binding. **At least one binding passes the S-set with carry-through perfect** — this is the `summarize` binding, and there is no fallback posture that lets an ungraded model serve the call (EVALS §3 step 5).
