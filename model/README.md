...
4. **`BUILD.md`** — the ordered plan. **Dependency-honest:** most steps want the *built* harness (real prompts against real traffic); only the eval scaffold and seed sets are useful before it. Step 0 also carries the **snapshot-custody mechanism** — part of **FD-84 (PROPOSED)**, as Step 0 itself records; how a recorded firing is made, kept and replayed keylessly — and the footprint of the spike's eventual removal.
