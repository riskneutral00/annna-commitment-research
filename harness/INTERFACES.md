...
- The harness only guarantees the tool contract carries **declared typed values**; it hands raw operands + a type_spec to the engine and stores the returned TypedValue/Handle. **For the `instant`/`interval` rows the `type_spec` carries the resolution context** `{zone, reference_instant, locale}` *(FD-27, stated 2026-08-21 — the harness supplies them from the board's stored zone, the firing's clock, and FR15's stored language; the two-argument pin is unchanged)*.
