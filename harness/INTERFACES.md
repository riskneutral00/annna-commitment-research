...
### 2.1 Normalize
```
normalize(utterance | trigger_event, context) -> { intent, fields:{…raw…}, ambiguities:[…] }
```
- Turns an utterance — or, on a trigger firing, the **trigger event** itself *(the union stated 2026-08-21: trigger firings have no utterance, and a synthetic one would be fabricated owner-tagged text; the seam stays at four calls)* — into a candidate structured action + raw field values. **Raw values are not trusted** — correctness-critical ones go through `typed_value`/`calculate` before commit.
...
```
- **The layer-2 call.** The law is `../security/SPEC.md §5` (FD-2) and lives there; this is its seam contract. Called by **context assembly** (`SPEC.md §8`), on every channel the layer-2 property names — free text whose author is not the owner (guest text, guest-uploaded documents of any class, connector-sourced text, surface-arrived text), plus owner-uploaded SOPs — **before** the privileged call, so the raw stranger text never reaches the tool-bearing model of §2.1.
...
- **Delivery outcomes are events the harness stores, not app decisions** *(2026-08-21, what makes D22–D23 constructible)*: a `notify_and_await` call's outcome — `sent`, `delivered-failed`, `handed-to-owner` — returns on this seam and the **harness** writes it as an engine-stored fact through `commit` (no new verb; the app records nothing but the raw result, `../app/SPEC.md §6`). A **`complaint`** arrives later and out of band; the app surfaces it through this same event shape and the harness stores it identically. Suppression (`SPEC.md §3.11`) is then a pure read over those stored events. `unavailable | timeout` members apply to the outward calls exactly as to §1's (the paragraph above §2).
...
- **The tool contract has a stated compatibility policy (RQ-13).** A published policy states what constitutes a breaking change to a verb, what notice a credential holder gets, and how long a superseded shape remains callable; a change that breaks a caller cannot land without satisfying it; and it is forward-only in the corpus's sense — a change never alters what an existing call already meant. The policy is authored at `BUILD.md` Step 8, **before any credential is issued** — cheap while the sole holder is the founder, expensive after (`../PRD.md §7`) — and gated at `SCENARIOS.md` X7 rather than left as prose. **What counts as breaking includes widening a closed shape**: a new status value, a new structured-decline reason, a new `labels[]` member — clients branch on closed enums (`SPEC.md §5`'s declines are designed to be branched on), so widening one is a breaking change unless the policy says otherwise. When authored it is spec-class markdown, on the `../deployment/egress-allowlist.md` precedent: the permission lands first, alone, where it is the only thing to read.
