...
3. **`SCENARIOS.md`** — the pass/fail acceptance suite (TDD substrate). `[MUST]` = build-gating (the held-out J1–J4 probes retired with Situation E, FD-109).
...
**Why the package stops at five files.** Five is the law and the law has one home: `../AGENTS.md` §Package shape names the set, and `../deployment/scripts/package-shape.mjs` enforces it over every package. Changing the count is a change to that section and that script — a ruling and a different piece of work, not a question to raise here. The labels this package's files use without re-definition are below; the history behind its decisions is at the end.

**Fixed dispatch and adaptive proposal — the one sentence, and where it lives.** Every executed step is routed by the harness; on an authorized owner turn the model may **propose** the next permitted action, which the harness validates and dispatches or records refused, while a structured trigger firing stays fixed-routed. The normative home is `SPEC.md §4a` (FD-105 amending FD-86) — read it there rather than from this page; this line is a pointer, not a second home (FR13).
