...
**Adding the rest of the team.** As an administrator she can bring others in under Hug Ocean — another front-desk admin, a boat leader, eventually the owner — each with their own access. *(Told in `operator-dive-center-second-seat.md` and run in `situation-6.md` — Fon takes the evening desk, Gop works the pier.)*

**The customer form.** annnä assembles the intake — contact, passport, the languages they speak, emergency contact, medical questionnaire, waiver, gear sizes, safety info — one form, saved once and reused. She sets the medical rule: any "yes" is a hard stop, no payment until a doctor's note is uploaded and handed in. **And she pays the language cost, out loud** *(the beat behind the corpus's gap 3 — FD-56: hand-authored, never model-translated)*: the form will render in Hug's four customer languages, so TingTing authors the guest-facing copy for each — Thai she writes herself in a sitting, English mostly holds from the template, and the two Chinese forms she takes to Wang Fang over two slow afternoons, field label by field label, ranked so Thai fills any hole. It is real work, done once — the standards' medical questionnaire arrives already translated; only Hug's own words need Hug. annnä builds the no-account customer page and freezes it, so every customer gets the identical thing.
