...
The **same stretch run the way a rental shop runs today**: a paper ledger, a WhatsApp thread, keys on a hook, deposits in an envelope. Ploy "holds" a bike by remembering to; two walk-ins want the same one; she double-books an NMAX she'd already promised from memory; the buffer lives in her head; a passport photo sits in a drawer.
