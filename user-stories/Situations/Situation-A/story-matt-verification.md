...
| [S18](story-matt.md#s18) | no layer row cites S18 yet — acceptance owed at [harness](#trace-harness), [engine](#trace-engine), [app](#trace-app), [model](#trace-model). |
...
## Bounded cases — 109 obligations, three of them retired
...
**FD-106.** MT-82, MT-83 and MT-84 were carried only by the group scenes FD-106 retired; they stay defined as retired rows so the series runs contiguously, are never reused, and their law stays story-owed at the layer rows their traces name. MT-85, MT-88 and MT-89 are recast to Matt's own race, ledger and payment records. MT-91–MT-109 append the obligations of the scenes FD-106 added.
...
| MT-88 | <a id="mt-88"></a> Run the October and November sequence of the [ledger](#october-ledger): per-lesson bookings, the lost race, moves, the COUNT refusal and retry, no-shows under the stored rule, and the package months. | Check every per-lesson student's count/minutes after each accepted act, the teacher interval once, bound totals, reminders and history. October final: Jasmine 4/240; Kai 4/225 under the ⟦keep⟧ assumption (2/105 if the answer is release both). November at S21's end: Kai 3/180, Jasmine 4/240. Package months carry no count or minute limit; their bill is the package. | Tests accumulated state across the actual full story. **Trace:** [harness](#trace-harness) D18 → Step 3 · [engine](#trace-engine) A1, Q1 → Steps 3, 4, 5 · [app](#trace-app) G10 → Step 5. |
| MT-89 | <a id="mt-89"></a> Record a payment from Matt's report — a package month and per-lesson lessons; repeat with no human report and with a guest attempting to mark another person's payment. | Correct attributed facts and scope; a package payment settles that month's package bill, not a lesson; no automatic verification, split or transfer, no forged reporter; an unrecorded payment reads unpaid, never inferred paid. | Tests recorded facts versus external events ([S15](story-matt.md#s15), [S16](story-matt.md#s16)). **Trace:** no layer row cites MT-89 yet — acceptance owed at [harness](#trace-harness), [engine](#trace-engine). |
...
| MT-94 | <a id="mt-94"></a> Matt switches the standing-slot option on, then enters Mark's weekly Thursday 09:00–11:00 and Georgina's Monday-and-Thursday 19:00–20:00 on package terms, with Messenger as contact and meeting link; separately a series with no package, and the option switched off afterwards. | Each week's lesson becomes its own booking as its date comes, carrying the package and the meeting link; October's fifth Thursday is listed skipped, not deleted; the same address stored as contact and meeting link stays two fields; switching the option off leaves series already entered untouched. | Tests the standing-slot series and its terms at entry ([HR-34](#hr-34); [S07](story-matt.md#s07), [S08](story-matt.md#s08), [S09](story-matt.md#s09)). **Trace:** no layer row cites MT-94 yet — acceptance owed at [harness](#trace-harness), [engine](#trace-engine), [app](#trace-app), [marketplace](#trace-marketplace). |
| MT-95 | <a id="mt-95"></a> On 21 October Matt asks “Who owes me what for October?”; repeat with a payment missing and after a cancelled lesson. | One tab per student, read from the record: package tabs show the package and the lessons it covers with no per-lesson price; per-lesson tabs sum each lesson (Kai NT$5,625, Jasmine NT$6,000, the cancelled 17 October at no charge); nothing is computed from an unfilled ⟦placeholder⟧. | Tests the owner's aggregate ledger surface, COVERAGE's owed “who owes me what, this month, right now” ([S15](story-matt.md#s15)). **Trace:** no layer row cites MT-95 yet — acceptance owed at [harness](#trace-harness), [engine](#trace-engine), [app](#trace-app), [model](#trace-model). |
| MT-96 | <a id="mt-96"></a> Mark asks for the fifth Thursday, 29 October; Georgina does not; repeat with the request arriving after the day has passed. | The fifth day is skipped by default and added only on the named student's request, charged at that package's hourly rate (2 hours, ⟦NT$1,000⟧) on top of the ⟦NT$4,000⟧ package; Georgina's lesson that evening stays skipped; the confirmation is text written for Matt to paste. | Tests the stored fifth-lesson-day rule and its per-student scope ([HR-35](#hr-35); [S07](story-matt.md#s07), [S15](story-matt.md#s15)). **Trace:** no layer row cites MT-96 yet — acceptance owed at [harness](#trace-harness), [engine](#trace-engine), [app](#trace-app). |
| MT-97 | <a id="mt-97"></a> Matt reports Mark's transfer; later asks whether Georgina has paid after the due date, accepts a written reminder and pastes it, then reports her transfer. | Each payment is a Matt-reported record with its amount, month and date; an unrecorded package reads unpaid, never assumed paid; the reminder is text annnä wrote, with no delivery state; no money passes through annnä. | Tests money recorded, never moved, and the reminder written, not sent ([S15](story-matt.md#s15), [S16](story-matt.md#s16)). **Trace:** no layer row cites MT-97 yet — acceptance owed at [harness](#trace-harness), [engine](#trace-engine), [app](#trace-app), [security](#trace-security). |
| MT-98 | <a id="mt-98"></a> Matt cancels Georgina's Monday, 19 October; test This occurrence, The series, and a stale selection. | annnä asks occurrence or series before acting; this occurrence releases 18:55–20:05 and the 18:45 reminder, reads “19 October cancelled by Matt” on the series, leaves the ⟦NT$4,000⟧ package unchanged and creates no credit; the rebook link offers one in-package lesson this month, and Georgina's own choice, Tuesday 20 October 19:00–20:00, is attributed to the link. | Tests owner cancellation of one package-series occurrence and the in-package rebook ([S16](story-matt.md#s16)); the rebook allowance's stored shape is an open question at [HR-35](#hr-35). **Trace:** no layer row cites MT-98 yet — acceptance owed at [harness](#trace-harness), [engine](#trace-engine), [app](#trace-app), [security](#trace-security). |
...
| MT-101 | <a id="mt-101"></a> Kai misses a per-lesson lesson on 22 October with no no-show rule stored; Matt answers; Kai misses again on 29 October. | The first report asks once — keep the charge and the count, or release both — and stores the answer on the template for per-lesson students with its date; the second report follows it silently and names the rule; neither report messages Kai. This companion assumes ⟦keep⟧; release both gives Kai 2/105 and NT$2,625 for October. | Tests a user's rule asked once and reused ([HR-37](#hr-37); [S17](story-matt.md#s17)). **Trace:** no layer row cites MT-101 yet — acceptance owed at [harness](#trace-harness), [engine](#trace-engine), [app](#trace-app), [model](#trace-model). |
| MT-102 | <a id="mt-102"></a> On 30 October Matt raises the per-lesson rate to ⟦NT$1,600⟧ with Replace existing while Kai holds a 5 November booking at NT$1,500. | Bookings made after the change quote the new rate; the booked 5 November lesson keeps NT$1,500; the compatibility check finds no booking outside the new rules; nothing in October is re-priced. | Tests the forward-only rule change; the same check that blocks [S13](story-matt.md#s13) passes here ([S18](story-matt.md#s18)). **Trace:** no layer row cites MT-102 yet — acceptance owed at [harness](#trace-harness), [engine](#trace-engine), [app](#trace-app). |
| MT-103 | <a id="mt-103"></a> Matt changes the package price list while Mark's and Georgina's series renew monthly and November's lessons are not yet made. | annnä asks which terms future months take — the package price they started on or the new list — and does not treat the list edit as the answer; the answer is stored on each series with its date; November's lessons take it when made; October's bills are outside the question. | Tests the mid-series ask and its stored answer ([S18](story-matt.md#s18), [S20](story-matt.md#s20)). **Trace:** no layer row cites MT-103 yet — acceptance owed at [harness](#trace-harness), [engine](#trace-engine), [app](#trace-app), [model](#trace-model). |
...
| MT-105 | <a id="mt-105"></a> The month turns to 1 November with no act from Matt. | October closes with its entries as recorded; Kai's and Jasmine's limits stand full, and Kai's pre-booked 5 November lesson counts 1 of 4; Mark's and Georgina's packages renew under the stored 30 October answer; Monday 30 November, Georgina's fifth Monday, is skipped by the fifth-lesson-day rule. | Tests the month boundary and renewal ([HR-39](#hr-39); [S20](story-matt.md#s20)); the fifth-Monday reading of the rule is an open question at [HR-35](#hr-35). **Trace:** no layer row cites MT-105 yet — acceptance owed at [harness](#trace-harness), [engine](#trace-engine), [app](#trace-app). |
| MT-106 | <a id="mt-106"></a> “What did October make?” is asked in November; then an edit to an October entry is attempted from the answer. | The answer reads from the record — Mark ⟦NT$5,000⟧, Georgina ⟦NT$4,000⟧, Kai NT$5,625 under ⟦keep⟧, Jasmine NT$6,000, total ⟦NT$20,625⟧ — with nothing invented and nothing editable from the answer; asking changes no entry. | Tests the history query ([S20](story-matt.md#s20)). **Trace:** no layer row cites MT-106 yet — acceptance owed at [engine](#trace-engine), [app](#trace-app), [model](#trace-model). |
...
| CRUD-23 | <a id="crud-23"></a> Reported actual end — attributed owner | Shown [S14a](story-matt.md#s14a): Kai's reported end with Matt as source; [S17](story-matt.md#s17): no-shows reported by Matt; [S15](story-matt.md#s15)/S16: payments reported separately. | Required: explicit correction with correct booking/date and retained attribution. | Required: withdraw a mistaken report through an explained corrective action; recompute dependent state using its contract. This is not authority to rewrite/delete the history of who said what. Permanent erasure remains pending. MT-80. **Trace:** no layer row cites CRUD-23 yet — acceptance owed at [harness](#trace-harness), [engine](#trace-engine). |
...
The main story chooses Replace existing, in [S13](story-matt.md#s13) and again for the rate in [S18](story-matt.md#s18). Candidate changes remain a draft until compatibility checks and any blockers are resolved. Any accepted change preserves the correct provenance, old terms/history and scope of affected uses.
...
Changing caps or other rules uses the same process when it creates a real conflict. A proposal's “why blocked” must come from validated constraints and actual records, not an LLM's invented reason. Completed historical records are not rewritten to make a new prospective rule true. [S18](story-matt.md#s18)'s rate change runs the same check and finds no booking outside the new rules: the check fails one change and passes another (ST1-R02).
...
The constraints are independent: four30-minute bookings use only2hours but exhaust count; two2-hour bookings plus one1-hour booking reach5hours before count4. Admission checks both atomically, including concurrent requests and existing legitimate holds according to their contract. Rescheduling one lesson does not count it twice. The proposed [S07](story-matt.md#s07) extension now explicitly elicits whether pre-start cancellation releases count/time; the worked owner selects Release allowance. For Jasmine, [S14b](story-matt.md#s14b) goes 2 lessons/120 minutes → 3/165 on Saturday booking and remains 3/165 on its move. A separate Sunday booking gives 4/225; a Wednesday request would be 5/285 and refuses COUNT, remaining 4/225. Keep is unchanged; Saturday cancellation gives 3/180; a fresh Wednesday retry gives 4/240. The portable ledger records each act. Monday's and Friday's lessons are already included. The limits bind per-lesson students; package students hold package hours instead ([HR-35](#hr-35)). This is a user-selected example, not a universal cancellation rule. [S17](story-matt.md#s17) stores a per-lesson no-show answer once ([HR-37](#hr-37)) and [S20](story-matt.md#s20) turns the month ([HR-39](#hr-39)); the rule contracts behind both, other cancellation states and scope-change handling are still owed before claiming complete enforcement.
...
## HR-35 — a monthly package: its terms, attendance and the fifth lesson day
...
Matt's price list offers packages — 1, 2, 3 or 4 hours a month, or 10 hours a year — beside the per-lesson rate. Mark and Georgina each hold two Red Eye packages, 4 hours each, 8 hours a month. **Every package price is a founder ⟦placeholder⟧**; no case computes an unfilled price. [S07](story-matt.md#s07) elicits and stores two package rules: attendance never changes a package bill, and a fifth lesson day in a month is skipped by default and, on the student's request, added at the package's hourly rate. [S15](story-matt.md#s15) applies the second to Mark's 29 October, [S17](story-matt.md#s17) the first to his no-show, and [S16](story-matt.md#s16) cancels an occurrence with the package unchanged.
...
Messages to students in this story — Mark's fifth-Thursday confirmation ([S15](story-matt.md#s15)), Georgina's cancellation with its rebook link and her payment reminder ([S16](story-matt.md#s16)), and Jasmine's offer ([S21](story-matt.md#s21)) — are written by annnä and copied by Matt into Messenger or Instagram himself. The proposal names what it will write; Copy message is the only act. No send grant, delivery state, read receipt or suppression is used or shown, because nothing was delivered (`user-stories/README.md` named gap 1 keeps the depicted-send distinction).
...
## HR-37 — a no-show rule the user has not set is asked once, then kept
...
A no-show is a human report, never inferred from a missed join or time passing ([HR-26](#hr-26)). For a package, the stored attendance rule answers it without a question ([S17](story-matt.md#s17), Mark). For a per-lesson student with no rule, annnä asks the owner once — keep the charge and the count, or release both — says the answer will be kept for later per-lesson no-shows, and stores it on the template with its date. The next report ([S17](story-matt.md#s17), Kai on 29 October) follows it silently and names it.

The worked answer is a founder ⟦placeholder⟧; the [ledger](#october-ledger) assumes keep and states the release-both result beside each affected row. Asking is the product's question to its own user, not a founder-wide default. **Open engine question, recorded (FD-106):** the stored rule scopes to every per-lesson student of this template, and its storage home is owed.
...
|---|---|---|---|
| ST1-R01 | <a id="st1-r01"></a> coordinator/third-party authority — Adapt; retired (FD-106) | The stored coordinator authority this finding adapted belonged to the retired group scene S13d. That law stands story-owed at its homes; the disposition is kept as provenance. | [HR-30](#hr-30) (retired); [MT-83](#mt-83) (retired); [XR-03](#xr-03) (retired) |
| ST1-R02 | <a id="st1-r02"></a> blocking versus allowing edits — Decline as contradiction | [S13](story-matt.md#s13) narrows a window and has an actual conflict. [S18](story-matt.md#s18) raises the per-lesson rate and the check passes: “The compatibility check finds no booking outside the new rules.” The same check can correctly fail one change and pass another. Do not replace the founder's actual-conflict blocker with a weaker rule or a blanket exemption for all price/format changes. | [HR-21](#hr-21); [MT-53](#mt-53); [S13](story-matt.md#s13)/S18 |
...
| ST1-R04 | <a id="st1-r04"></a> booked roster versus actual attendance — Accept clarification | Keep the agreed quote or package distinct from later reported attendance: a no-show is a reported fact, and what it costs is the user's stored rule — attendance never changes a package bill, and the per-lesson answer is asked once. The group price-table wording this finding first targeted retired with the group scenes (FD-106). The mechanism for an explicitly agreed change to an already-bound quote still needs a specified version/replacement contract; do not assume the existing engine permits overwriting its stored price. | [HR-35](#hr-35); [HR-37](#hr-37); [MT-89](#mt-89); [S15](story-matt.md#s15)/S17 |
...
| ST1-R06 | <a id="st1-r06"></a> invitation/manage link — Adapt | Clarify which booking/request and actions each recipient link permits, then add expiry/revocation/forwarding cases. The report applies a bare-entry-link rule to Kai's explicitly individual link. The source also permits per-recipient tokens to manage their own bookings. Its suggested claim that cancellation makes the booking cease to exist is wrong: cancellation preserves a record. | [HR-18](#hr-18); [MT-13](#mt-13); [MT-62](#mt-62); [MT-79](#mt-79); [XR-04](#xr-04) |
...
| ST1-R08 | <a id="st1-r08"></a> coordinator's contact channel — Adapt; optional short addition | Explain the actual manual handoff and unavailable automated route when useful. Do not paste the suggested sentence: Kai has a LINE contact in the story, not the email the review proposes using. A recorded channel value also does not establish a matching send grant or a working integration. No current external-provider claim was independently researched in this pass. | [HR-16](#hr-16); [HR-24](#hr-24); [MT-46](#mt-46); [S10](story-matt.md#s10)/S16 |
...
The published treatment of ST1-R08 is the manual-handoff clarification only. ST1-R09’s optional student-reminder expansion is not selected. These decisions avoid substituting an unprovided email for a student's messaging contact, treating contact data as a grant, or presenting dated provider claims as newly researched facts. The new console is the approved story’s direction; FD-43/54’s “no agent appears” paragraph covers guest manage state, not a pre-existing guest agent. [S13](story-matt.md#s13)’s actual conflict and [S18](story-matt.md#s18)’s compatible rate change use the same blocker discipline; there is no blanket price/format exemption.
...
|---|---|---|---|
| RS-01 — student reaches monthly limit | Adapted and shown | The fourth request is valid. Separate Sunday acceptance reaches 4/225; Wednesday would be 5/285 and refuses COUNT; cancellation releases Saturday and fresh retry succeeds; [S20](story-matt.md#s20) opens November's allowance to an ordinary booking. No invented free November slot or reset-driven automatic booking. | [MT-52](#mt-52)/55/76/88; [S14b](story-matt.md#s14b); October ledger. |
| RS-02 — guest in another timezone | Accepted and shown with explicit display choice | After Thursday confirmation, Kai selects Asia/Tokyo and sees 17:30–18:15 beside 16:30–17:15 Asia/Taipei, then returns to Taipei. Earlier typed times are never silently reinterpreted. | [XR-01](#xr-01); [MT-75](#mt-75); S11. |
| RS-03 — student reminders / unavailable channels | <a id="rs-03"></a> Optional extension not selected | Matt’s forgetting motivates his own selected reminder. Student recipients, contacts, timing, grants and actual provider state need elicitation and verification before any added send path. Existing manual handoff remains useful. | [MT-46](#mt-46)/58/60; [HR-16](#hr-16)/24; [S07](story-matt.md#s07)/S10/S16. |
...
| RS-05 — group disagreement | Retired with the group scenes (FD-106) | The group-disagreement obligation belonged to the retired group scenes; its law stays story-owed at its homes. | [XR-03](#xr-03) (retired); [MT-83](#mt-83) (retired); [HR-30](#hr-30) (retired). |
...
**Rebuilt under FD-106.** The ledger follows Matt's own practice: two per-lesson students whose monthly count and time limits are tracked, and two package students whose bill is the package. The LED labels are fixture row identifiers re-cut in this rebuild — a pre-FD-106 LED number named a different row, and git history keeps that table. **Every ⟦bracketed⟧ value is a founder placeholder. The ⟦keep⟧ assumption is explicit:** Matt's per-lesson no-show answer in [S17](story-matt.md#s17) is a placeholder, these rows assume *keep the charge and the count*, and every affected row states the *release both* result beside it.

Per-lesson tables follow narrative action order, not lesson start order. Count/minutes belong to the same known student and the confirmed English offering, by the lesson month on Matt’s Asia/Taipei calendar. Buffers do not consume student minutes. A refused row’s deltas are the attempted increment; its after-state stays unchanged. Moves, keep actions, reports and display edits add no usage. Outcome codes are documentary labels, not new engine API vocabulary.

### October — per-lesson students

| ID | Scene | Student | Count before | Minutes before | Count delta | Minutes delta | Count after | Minutes after | Outcome | Reason |
|---|---|---|---|---|---|---|---|---|---|---|
| LED-01 | [S11](story-matt.md#s11) | Kai | 0 | 0 | 1 | 45 | 0 | 0 | conflict | Stale Thu 15 October 16:05–16:50 fails after the appointment extension; no quota use. |
| LED-02 | [S11](story-matt.md#s11) | Kai | 0 | 0 | 1 | 45 | 1 | 45 | accept | Fresh Thu 15 October 16:30–17:15, TWD 1125. |
| LED-03 | [S11](story-matt.md#s11) | Kai | 1 | 45 | 0 | 0 | 1 | 45 | unchanged | Tokyo readback and return to Taipei change display only. |
| LED-04 | [S12](story-matt.md#s12) | Jasmine | 0 | 0 | 1 | 60 | 1 | 60 | accept | Mon 12 October 13:00–14:00 from her Instagram link, TWD 1500. |
| LED-05 | [S13](story-matt.md#s13) | Jasmine | 1 | 60 | 0 | 0 | 1 | 60 | unchanged | Agreed move 13:00–14:00 to 15:30–16:30 keeps count, minutes and TWD 1500. |
| LED-06 | [S13g](story-matt.md#s13g) | Jasmine | 1 | 60 | 1 | 60 | 2 | 120 | accept | Wins Fri 16 October 14:00–15:00, TWD 1500. |
| LED-07 | [S13g](story-matt.md#s13g) | Kai | 1 | 45 | 1 | 60 | 1 | 45 | conflict | Losing Fri 16 October 14:00–15:00 attempt draws no allowance. |
| LED-08 | [S13g](story-matt.md#s13g) | Kai | 1 | 45 | 1 | 60 | 2 | 105 | accept | Fresh Fri 16 October 16:00–17:00, TWD 1500. |
| LED-09 | [S14a](story-matt.md#s14a) | Kai | 2 | 105 | 0 | 0 | 2 | 105 | unchanged | Matt reports Thursday's end at 17:15; the booked duration and quote stand. |
| LED-10 | [S14b](story-matt.md#s14b) | Jasmine | 2 | 120 | 1 | 45 | 3 | 165 | accept | Sat 17 October 10:00–10:45, TWD 1125. |
| LED-11 | [S14b](story-matt.md#s14b) | Jasmine | 3 | 165 | 0 | 0 | 3 | 165 | unchanged | Move to 10:15–11:00, reminder 10:00; still one 45-minute booking. |
| LED-12 | [S14b](story-matt.md#s14b) | Jasmine | 3 | 165 | 1 | 60 | 4 | 225 | accept | Separate Sun 18 October 10:00–11:00 is the fourth lesson, TWD 1500. |
| LED-13 | [S14b](story-matt.md#s14b) | Jasmine | 4 | 225 | 1 | 60 | 4 | 225 | COUNT | Separate Wed 21 October 10:00–11:00 would be 5/285; count exceeds 4 while 75 minutes remain below 300. |
| LED-14 | [S14b](story-matt.md#s14b) | Jasmine | 4 | 225 | 0 | 0 | 4 | 225 | unchanged | Keep booking abandons cancellation; Saturday stays confirmed. |
| LED-15 | [S14b](story-matt.md#s14b) | Jasmine | 4 | 225 | -1 | -45 | 3 | 180 | accept | Saturday cancellation releases allowance under Matt’s chosen pre-start policy; reservation and pending reminder stop. |
| LED-16 | [S14b](story-matt.md#s14b) | Jasmine | 3 | 180 | 1 | 60 | 4 | 240 | accept | Fresh Wednesday retry succeeds at 10:00–11:00, TWD 1500; Saturday stays cancelled. |
| LED-17 | [S15](story-matt.md#s15) | Kai | 2 | 105 | 1 | 60 | 3 | 165 | accept | Thu 22 October 17:00–18:00, from a LINE request after his Friday lesson, TWD 1500. |
| LED-18 | [S15](story-matt.md#s15) | Kai | 3 | 165 | 1 | 60 | 4 | 225 | accept | Thu 29 October 17:00–18:00, TWD 1500. |
| LED-19 | [S17](story-matt.md#s17) | Kai | 4 | 225 | 0 | 0 | 4 | 225 | unchanged under ⟦keep⟧ | 22 October no-show; the first per-lesson no-show asks Matt once. Under keep the charge and count stand. Release both: −1/−60 → 3/165. |
| LED-20 | [S17](story-matt.md#s17) | Kai | 4 | 225 | 0 | 0 | 4 | 225 | unchanged under ⟦keep⟧ | 29 October no-show follows the stored answer silently. Release both: −1/−60 → 2/105. |

### October — package students

Package months carry no count or minute limit. Two Red Eye packages of 4 hours each are 8 hours a month, and attendance never changes the bill ([HR-35](#hr-35)); lessons and hours are listed to show what the package covers, not to meter it.

| ID | Scene | Student | Act | Lessons · hours | Bill | Outcome |
|---|---|---|---|---|---|---|
| LED-21 | [S08](story-matt.md#s08) | Mark | Standing Thursday 09:00–11:00 entered: 1, 8, 15 and 22 October; 29 October skipped as the fifth Thursday. | 4 · 8 h | ⟦NT$4,000⟧ | accept |
| LED-22 | [S09](story-matt.md#s09) | Georgina | Standing Monday and Thursday 19:00–20:00 entered: 1, 5, 8, 12, 15, 19, 22 and 26 October; 29 October skipped as the fifth Thursday. | 8 · 8 h | ⟦NT$4,000⟧ | accept |
| LED-23 | [S15](story-matt.md#s15) | Mark | Fifth Thursday, 29 October, added on his request at the package hourly rate, 2 hours. | 4 + 1 added · 10 h | ⟦NT$4,000⟧ + ⟦NT$1,000⟧ = ⟦NT$5,000⟧ | accept |
| LED-24 | [S15](story-matt.md#s15) | Mark | Transfer reported by Matt. | 4 + 1 added · 10 h | ⟦NT$5,000⟧, paid | recorded |
| LED-25 | [S16](story-matt.md#s16) | Georgina | Monday 19 October cancelled by Matt, this occurrence only; 18:55–20:05 and the 18:45 reminder released; no credit. | 7 scheduled · 7 h | ⟦NT$4,000⟧, unchanged | accept |
| LED-26 | [S16](story-matt.md#s16) | Georgina | Rebooks Tuesday 20 October 19:00–20:00 through the rebook link, inside the package. | 8 · 8 h | ⟦NT$4,000⟧, unchanged | accept |
| LED-27 | [S16](story-matt.md#s16) | Georgina | Past ⟦the day package payments are due⟧ with no payment recorded; annnä writes a reminder and Matt pastes it. | 8 · 8 h | ⟦NT$4,000⟧, unpaid | unchanged |
| LED-28 | [S16](story-matt.md#s16) | Georgina | Transfer reported by Matt. | 8 · 8 h | ⟦NT$4,000⟧, paid | recorded |
| LED-29 | [S17](story-matt.md#s17) | Mark | 22 October no-show reported by Matt; no question asked. | 4 + 1 added · 10 h | ⟦NT$5,000⟧, unchanged | unchanged |

### November — per-lesson students

| ID | Scene | Student | Count before | Minutes before | Count delta | Minutes delta | Count after | Minutes after | Outcome | Reason |
|---|---|---|---|---|---|---|---|---|---|---|
| LED-30 | [S18](story-matt.md#s18) | Kai | 0 | 0 | 1 | 60 | 1 | 60 | accept | Thu 5 November 17:00–18:00, booked in October at TWD 1500; the 30 October rate change leaves it at 1500. |
| LED-31 | [S20](story-matt.md#s20) | Jasmine | 0 | 0 | 1 | 60 | 1 | 60 | accept | The allowance stands full at the month turn; Mon 2 November 15:30–16:30 at ⟦NT$1,600⟧. |
| LED-32 | [S21](story-matt.md#s21) | Kai | 1 | 60 | 1 | 60 | 2 | 120 | accept | Thu 19 November 17:00–18:00 at ⟦NT$1,600⟧, on the board by the full Thursday. |
| LED-33 | [S21](story-matt.md#s21) | Jasmine | 1 | 60 | 1 | 60 | 2 | 120 | accept | Mon 9 November 15:30–16:30 at ⟦NT$1,600⟧. |
| LED-34 | [S21](story-matt.md#s21) | Jasmine | 2 | 120 | 1 | 60 | 3 | 180 | accept | Mon 16 November 15:30–16:30 at ⟦NT$1,600⟧. |
| LED-35 | [S21](story-matt.md#s21) | Jasmine | 3 | 180 | 1 | 60 | 4 | 240 | accept | Fri 27 November 14:00–15:00 at ⟦NT$1,600⟧. |
| LED-36 | [S21](story-matt.md#s21) | Kai | 2 | 120 | 1 | 60 | 3 | 180 | accept | Thu 26 November 17:00–18:00 at ⟦NT$1,600⟧, booked while Matt looks at 19 November. |
| LED-37 | [S21](story-matt.md#s21) | Jasmine | 4 | 240 | 0 | 0 | 4 | 240 | unchanged | 02:40 request to move 27 November to Mon 30 November 13:00; the unattended run parks at its budget and nothing moves. |
| LED-38 | [S21](story-matt.md#s21) | Jasmine | 4 | 240 | 0 | 0 | 4 | 240 | unchanged | After Matt's OK and the pasted offer she moves 27 November to Mon 30 November 15:30–16:30 herself; still one lesson at ⟦NT$1,600⟧. |

### November — package students

| ID | Scene | Student | Act | Lessons · hours | Bill | Outcome |
|---|---|---|---|---|---|---|
| LED-39 | [S20](story-matt.md#s20) | Mark | Package renews under the 30 October answer: Thursdays 5, 12, 19 and 26 November. | 4 · 8 h | ⟦November package price under S18's answer⟧ | accept |
| LED-40 | [S20](story-matt.md#s20) | Georgina | Package renews under the same answer: Mondays 2, 9, 16 and 23 and Thursdays 5, 12, 19 and 26 November; Monday 30 November skipped as a fifth Monday (open question, [HR-35](#hr-35)). | 8 · 8 h | ⟦November package price under S18's answer⟧ | accept |

### Final state of the full story

| Student | Terms | October | November, at S21's end |
|---|---|---|---|
| Kai | Per lesson: NT$1,500 an hour, ⟦NT$1,600⟧ for bookings after 30 October | 4 lessons / 225 minutes, NT$5,625, under ⟦keep⟧ (release both: 2 / 105, NT$2,625) | 3 / 180: 5 November at NT$1,500; 19 and 26 November at ⟦NT$1,600⟧ |
| Jasmine | Per lesson | 4 / 240, NT$6,000; 17 October cancelled before it started, no charge | 4 / 240 at ⟦NT$1,600⟧: 2, 9, 16 and 30 November (moved from 27) |
| Mark | Two Red Eye packages, 8 hours a month | Thursdays 1, 8, 15, 22 (no-show) and 29 (added): ⟦NT$5,000⟧, paid | Thursdays 5, 12, 19, 26: ⟦November package price under S18's answer⟧ |
| Georgina | Two Red Eye packages, 8 hours a month | 8 lessons (19 cancelled, 20 rebooked; 29 skipped): ⟦NT$4,000⟧, paid late after a pasted reminder | 8 lessons; 30 November skipped |
| **October total** | | **⟦NT$20,625⟧** under ⟦keep⟧ (release both: ⟦NT$17,625⟧) | |
...
All dates are October and November 2026 except the September landing week. Thursday 1, Monday 5, Monday 12, Thursday 15, Friday 16, Saturday 17, Sunday 18, Monday 19, Tuesday 20, Wednesday 21, Thursday 22, Monday 26, Thursday 29, Friday 30 and Saturday 31 October, and Sunday 1, Monday 2, Thursday 5, Monday 9, Thursday 19, Monday 23, Thursday 26, Friday 27 and Monday 30 November agree with their named weekdays. October has five Thursdays and four Mondays; November has five Mondays and four Thursdays. Landing week 13–19 September and later weeks 11–17 / 18–24 October are Sunday-through-Saturday displays. The narrative is a worked setup and follow-through, not an assertion that each scene occurs on the date currently being viewed.

| Commitment | Lesson / event | Reserved interval | Matt’s lesson reminder |
|---|---|---|---|
| Usual Run | 06:00–06:45 | 05:50–07:00 | None selected |
| Monday 12 October Run exception | 06:00–06:45 | 05:30–07:00, 30-minute total before-buffer | None selected |
| Saturday 17 October Run | Cancelled | 05:50–07:00 released | None selected |
| Saturday 31 October Run | 06:30–07:15 after the read-back correction; 18:30 never written | 06:20–07:30 | None selected |
| Gym task, Monday, Wednesday and Friday | Before-noon and afternoon windows | None — a Task holds no time | None selected |
| Personal appointment Thursday 15 October | Initially 15:00–16:00; later 15:00–16:15 | Same interval, zero own buffers | None selected |
| Mark, October Thursdays | 09:00–11:00 on 1, 8, 15, 22 and, added, 29 | 08:55–11:05 | 08:45 |
| Georgina, October Mondays and Thursdays | 19:00–20:00; Monday 19 cancelled; Tuesday 20 rebooked | 18:55–20:05; Monday 19 released | 18:45; Monday 19 removed |
| Jasmine Monday 12 October | Initially 13:00–14:00; moved 15:30–16:30 | Initially 12:55–14:05; then 15:25–16:35 | 12:45 replaced by 15:15 |
| Kai Thursday 15 October | 16:30–17:15 | 16:25–17:20 | 16:15 Taipei |
| Jasmine Friday 16 October | 14:00–15:00, online | 13:55–15:05 | 13:45 |
| Kai Friday 16 October | 16:00–17:00, online | 15:55–17:05 | 15:45 |
| Jasmine Saturday 17 October | 10:00–10:45 → 10:15–11:00 → cancelled | 09:55–10:50 → 10:10–11:05 → released | 09:45 → 10:00 → stopped |
| Jasmine Sunday 18 October | 10:00–11:00 | 09:55–11:05 | 09:45 |
| Jasmine Wednesday 21 October | 10:00–11:00, after successful retry | 09:55–11:05 | 09:45 |
| Kai Thursdays 22 and 29 October | 17:00–18:00, both reported no-shows | 16:55–18:05 | 16:45 |
| Mark and Georgina, November | Mark Thursdays 5–26 at 09:00–11:00; Georgina Mondays 2–23 and Thursdays 5–26 at 19:00–20:00; Monday 30 skipped | 08:55–11:05; 18:55–20:05 | 08:45; 18:45 |
| Kai Thursdays 5, 19 and 26 November | 17:00–18:00 | 16:55–18:05 | 16:45 |
| Jasmine Mondays 2, 9 and 16 November | 15:30–16:30 | 15:25–16:35 | 15:15 |
| Jasmine Friday 27 November | 14:00–15:00 → moved to Monday 30 November 15:30–16:30 | 13:55–15:05 → 15:25–16:35 | 13:45 → 15:15 |

Before the personal-event extension, English may end at 14:55 or start at 16:05. After extension the next possible start is 16:20; the offered 16:30 is valid without an earliest-option promise. Friday’s offered 16:00 is also valid. Sunday and Wednesday 09:55–11:05 reservations fit 09:00–23:00 and have no narrated conflicting commitment; Wednesday has no Run, Sunday is not a Run day, and Wednesday's gym window holds no time. Kai's Thursday reservations end at 18:05, before Georgina's 18:55, and every Monday lesson after the S13 change starts inside the 15:00–23:00 Monday window. Each time check here is documentary fixture arithmetic, not live availability or a provider route check. Preparation tasks have due times, not fabricated fixed-duration reservations.

<a id="quote-matrix"></a>

## Quote matrix — one price for the lesson

**FD-106.** The two-to-four-student tier in QUOTE-05–16 was carried by the retired group scenes; `engine/BUILD.md` keeps QUOTE-01–16 unchanged as calculation controls, so the rows stand, story-owed. Matt's current prices beyond NT$1,500 are founder ⟦placeholders⟧ and add no rows here. One student uses TWD 1500/hour; two, three or four students booked use TWD 2000/hour total. Multiply the hourly rate by duration/60, never by the participant count. The booked roster selects the quote tier. Actual attendance is a separate attributed fact and does not overwrite the agreed quote. Explicitly agreed commercial changes still need the bound-quote version/replacement contract named in layer debt.
...
The earlier expansion of Situation A's first telling was a 14-scene scripted draft with 15 edge cases. Its public-entry, empty-account, private save/reload/use, guest booking, binding-time and ordinary recorded-outcome journey led to the current Matt draft; it was never a replacement of that telling, which FD-106 retired on 2026-09-15 once its beats were re-told here or in Situation D. Its proposed owner preview, no-guest-console path, monthly base/100-per-hour/ten-hour example and initial no-agent-speech assumptions are not imported into Matt’s approved story. Earlier 2029 or Monday-15-October dates were superseded by the founder’s 2026/Monday-12-October direction. Prior “28 harness cues” and “24 CRUD families” counts were overtaken by 33/26, and under FD-106 by 42 cues and 26 families, two retired; the retained cases were 90, not the early Matt draft’s 17, and FD-106 brought them to 109 with three retired.
...
The old request “ok i'll review 1 story” authorized one example before wider expansion. Subsequent directions made Matt’s English teaching the working protagonist and authorized applying group additions before independent review; FD-106 later retired those group scenes. The earlier plan-only status and failed worktree setup record are historical: the latter refused because the project board was in investigate mode, after which the author drafted externally without bypassing it. This approved publication changes no board state and authorizes no wider story expansion. Unprovided native-iPhone, returning-account, successful-catalog and freelance-dive stories remain unelicited/unauthored work; FD-68’s remaining limit and the ER holdout stay intact.

### Legacy edge cases from the earlier draft — provenance, not new acceptance criteria

These source variations are retained for source completeness. Their vocabulary and rates belong to the earlier draft, not Matt’s fixture; FD-106 generalised the draft's character names here, and the hashed source keeps the original wording. Similar Matt cases do not prove their unique enrollment, publication or failed-move outcomes; the applicable existing layer homes still decide them.

| ID | Variation | What the owner or the student should observe |
|---|---|---|
| **E01** | Signup verification is interrupted, or the owner returns through a second browser. | She can complete the configured sign-in flow and reach the correct account. An interruption does not leave her believing an account or saved template exists when it does not. |
| **E02** | Catalog lookup fails instead of returning an empty result. | The app says it could not check the catalog. The owner can retry or choose to author from scratch; it invents neither a successful search nor a template. |
| **E03** | The owner says “six” without enough context to distinguish morning and evening. | The interpretation is visible. Where no defensible basis exists and the difference is material, the app asks. It does not conceal a guess inside a published booking rule. |
| **E04** | The owner sets a two-hour minimum inside a one-hour teaching window. | The incompatible configuration is explained before it is presented as usable availability. She can correct it or abandon the change. |
...
| **E06** | The owner closes an editor with unsaved changes, then returns. | The persisted version is clear. The story makes no promise of automatic recovery for an unsaved buffer. Saved work does survive leaving and signing back in. |
...
| **E08** | The student's booking response is lost and they submit again. | Recovery resolves the same booking attempt. No duplicate commitment, enrollment or money entry appears from the retry. |
| **E09** | The student opens an invalid or revoked link. | They get a generic dead end with no private data, no stale booking controls and no forced owner-account signup. Do not apply the separate24-hour unused-public-pass rule to every booking-management link. |
| **E10** | The replacement slot disappears while the student is moving their lesson. | Their original booking remains intact when the move refuses; the app does not turn an unsuccessful move into an accidental cancellation. |
| **E11** | The owner changes template pricing while the student is already booked. | She can distinguish the saved template revision, the setup used for future bookings, and the old terms bound to the student's existing commitment. |
| **E12** | The owner asks to publish “my whole setup” to the catalog. | The app distinguishes publishing an authored reusable shape from exposing a populated board. It refuses to package private commitments, customers, addresses or notes as public template content. |
| **E13** | The owner opens the template under another owner's account. | The other account cannot see or modify her private template. Signing back into the correct account recovers the correct saved work. |
| **E14** | The owner deletes a saved template or removes it from use after bookings exist. | The consequences are stated before she acts. Existing commitments and their history are not silently erased; deleting a saved original, removing a setup and unpublishing a listing are not conflated. |
...
Transfer census: **51 source artifacts**, including frozen duplicates; **90 MT cases, 26 CRUD rows, 33 HR cues, 15 finding dispositions, six scenario dispositions and eight LEGAL-01 work areas**. These are retention counts, not passing application checks. FD-106 later appended MT-91–109 and HR-34–42 and retired MT-82–84, HR-17/30/32 and CRUD-25/26; none of that is transferred source.
