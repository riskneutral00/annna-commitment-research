...
Every value below is enforced by the product's construction, not by policy. That is the family trait: **we don't promise behavior, we make the wrong thing impossible.**
...
2. **Your life is not content.** Private commitments block availability without ever being visible. Your students, renters, and customers see that you aren't free — never why. The people you transact with fill ordinary forms; they never make accounts, never meet a chatbot, never see your board.
3. **The math is never a guess.** Times, availability, capacity — every number comes from deterministic code, never from an AI's judgment. The agent talks; the engine answers. Same question, same answer, always.
...
- Not a calendar with a chatbot stapled on. The AI cannot author a number, a time, or a permission — by construction.
