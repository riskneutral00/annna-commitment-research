...
**One readiness condition joins the criteria (FD-63, 2026-08-22 — the founder-dogfood exit):** the founder's own real bookings run on the system with **zero fallbacks** to the old tools — binary, and it tests the completeness bet at the smallest scale the product has. It is a readiness condition, not a schedule (FR2). *(**Amended 2026-08-23, FD-79: dogfood and pilot are the same run, not sequential** — zero fallback over the founder's real teaching weeks **including his students' bookings**, evaluated continuously from first live use rather than over a solo run that precedes the pilot. His reason, verbatim: "right when I'm using an app for myself is when I'm going to be giving it to my students to interact with. I have no personal use case without my students." The original four-solo-weeks sequencing is struck: it described a state this product cannot reach, because a two-party commitment harness has no single-party mode to dogfood in. The condition is unchanged in kind — zero fallback, binary — and only its window and population move.)*

## Risks and the central bet

*The register. Each entry is a live risk, not a resolved one. The five PRD §4.2-era risks joined 2026-08-31 — **FD-101** ruled them carried as surviving review (FDR-13's selected arm), so they are register rows now, not an owed list.*

- **A public surface is a permanent surface** *(carried from `../PRD.md §9`, FD-101)* — an issued credential's contract shape is expensive to change; RQ-13's compatibility policy, gated at issuance, is the mitigation.
- **An admitted caller spends annnä's money** *(carried, FD-101)* — a runaway owner agent converts the model budget into someone else's retry loop; the per-firing budget, the external-caller rate posture and the refusal records are the mitigations.
- **The quarantine was designed against prose, and callers are not prose** *(carried, FD-101)* — RQ-8 extends the tagging door, not the control; the structured-parameter bypass is the cheap half, and the control's own posture is unchanged and must stay so.
- **A parity requirement can be satisfied downward** *(carried, FD-101)* — RQ-2 is true if the app is made poorer instead of the surface richer; the parity gate's both-directions set equality is the mechanical half of the mitigation.
- **The agent-first claim can outrun its scenarios** *(carried, FD-101)* — outward claims stay bound to landed scenario evidence; the VOICE derivation rule is the brake.
...
- **Legal.** annnä's role in the data its owners admit — the owner's business as controller and annnä as processor where that relationship applies — and eventually routing customers between businesses, are legal questions before engineering ones. A formal legal review is a hard build gate that green tests cannot argue past — the referral beat is deferred behind it (FR1). *(Corrected in place 2026-09-13 to its source, `../security/SPEC.md` §12's proposed roles — "Owner's business as controller; annnä as processor where that relationship applies" — whose professional review is owed at `../security/BUILD.md` Step 8; the prior premise, holding customers' passports and medical notes, was retired by the founder's v1 removal recorded at `../security/SPEC.md` §4 — "Owner and guest file uploads and participant-document authentication are excluded from v1." The gate itself is unchanged and no replacement outward copy is authored here; under the freeze's in-place truth-correction class, FD-101, remediation audit U19-B-01 with ROOT-03, unit U60.)*
...
- **A named pilot — ~~open ruling OR-40~~, CLOSED 2026-08-23 (FD-77)** *(FD-62, 2026-08-22 — promoted from never-authoritative `NOTES.md`, because the pilot is the only instrument the criteria above can run on and an instrument named nowhere authoritative cannot be owed)*: **OR-40 asked who the pilot cohort is. The answer: the founder's own English students**, interacting as guest parties through tokenized links — never accounts, which is what keeps the pilot inside the not-a-guest-facing-product law. Recruitment is stated as readiness, never schedule (FR2): it proceeds when the deployed first-phase slice is green and the founder's own live teaching week runs on annnä with student bookings flowing through it. The cohort size he is aiming at is a business target and is deliberately not stated as a number here *(softened 2026-08-31, m-38 — `../RULINGS.md`'s provenance record quotes the founder's own "10 users" remark, so "absent from every tracked file" was false; the register quotes, this file still declines to target)*. **What the closure unlocks:** the primary signals above become exercisable, and the FD-63 condition stops being the only runnable instrument.
