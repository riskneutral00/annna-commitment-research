...
> **What the freeze does not do:** it deletes no scenario, weakens no gate, and removes nothing from gate-coverage — **and two external gates adopt this package's frozen formats while it waits, named so the freeze's reach is honest** *(2026-08-31, F-27)*: harness **G8** (save-as-bundle projects a valid `SPEC.md §1.2` bundle) and the install-door validation the app/TDD **D3** walk drives (`BUILD.md` Step 0's validator) both gate on formats defined here; the freeze freezes work on this package, not those adoptions. A frozen spec is a spec waiting its turn, not a retired one. The app must keep working with this package **absent or unreachable** — the shipped four skins + Plain, every installed template, **and every installed pack's persisted device copy** keep working regardless (`../app/INTERFACES.md`; restated 2026-08-31, F-26 — the on-device skins were missing from this floor's wording), and that requirement is *not* frozen.
