...
Canned catalog (the four shipped skins + one store-skin fixture + the two seed templates — the Step 0 **throwaway fixtures**, FR38, never shipped catalog defaults), entitlement map covering the fixture for one account and not another, test signer minting genuinely expiring URLs (`INTERFACES.md §1`) — the same mock CI uses forever. Gate: **P1–P2, E1, E4**.
...
The browse riser, the install door wired to the real harness, the propose→confirm parameter walk, uninstall. Gate: **I1–I6, D3 (template half)**; re-run harness **B1/B2** unchanged — *zero new verbs is the exam*.
