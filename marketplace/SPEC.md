...
Author → validate against the format grammar → assign one category + tags (+ optional featured flag) → publish to the catalog. Skins additionally run the pack pipeline (`../assets/make-pack.mjs`) and register their derivatives behind entitlements — **into the closed service's private delivery, never a public web-root**: the pipeline's `public/assets/packs/` output path serves the shipped free skins only.
...
- **User supply.** The prior build's retrieval-corpus flywheel ("instructor #200 gets a good draft because #1–199 corrected theirs") presumed user publishing — recorded as a future direction, **not specced**, under the admin-only ruling. Note the dependency: the engine's no-export ruling holds *because* supply is admin-only (publishing is authored by the admin pipeline, never extracted from live boards); if user publishing ever opens (e.g. OR-28's link-share stage), the engine gains export pressure — re-open both together.
