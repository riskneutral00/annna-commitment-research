...
## Confirmed findings from the review — **two closed 2026-08-06 (F20, F7), three still awaiting rulings**
...
- **No marketplace language law — narrowed 2026-08-06 by FR15, still open.** The guest-facing half is now ruled: guest surfaces render in the **owner's stored language setting**, asked once at setup (`../app/SPEC.md §5`). What FR15 deliberately does **not** reach is annnä's own catalog surface — **category names, bundle `domain` display naming, and the D3 preview's plain-language rule lines**. Those are one global surface with no owner to ask, so no per-owner setting can answer them; they need a product-scope ruling. Open design item, correctly scoped.
...
- **No-show money** — who's owed what when the customer doesn't show; does the ledger encode the shop's own no-show rules? (Engine/harness product question, raised by the dive-pro test reader.)
...
## Prepared: the Situation-A′/C′ install-run probe
...
**Domain-brief questions for Matt before the probe runs** (the probe-elicit inputs):
1. Who actually finds the store first in each story — the owner browsing, or the agent suggesting it when the owner describes a need?
2. For A′: does Sofia become entitled to a store skin in the same visit, exercising the entitlement path end-to-end?
3. For C′: does Hug Ocean install into an empty account, or an account with existing boards? (Install-into-lived-in-board is the harder, truer case.)
