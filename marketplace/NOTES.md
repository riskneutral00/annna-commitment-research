...
**OR-28 — curation & impersonation.** *Decision:* how publisher identity is vouched when anything beyond founder supply exists (the "Official PADI Blueprint" problem). *Status (2026-08-24, FD-82):* **unruled; a future gate on top of owner-publish.** Admin-only supply no longer makes impersonation structurally impossible. *Options on record:* (a) fully curated catalog; (b) verified-publisher identity; (c) staged private link-share. *Evidence to gather before ruling:* trademark posture for certifying-body names; how the app stores label "official". *Trigger:* he decides to put authorization on top — not this sitting. **No-export is already split:** a published bundle is an authored blanked shape (P4); extracting a populated board stays refused.

**OR-29 — template supply terms.** *Decision:* what terms, if any, attach to template supply. *Status:* open, and the substance sits **inside the closed service** — per `SPEC.md §5` (FR18) the terms are not stated in this repo, so neither are the options. *Recorded dependency, which is the only part the open half needs:* terms sit in the closed service (OR-29) and may later gate *who pays*; they do not reopen no-export. FD-82 already opened owner-publish. OR-28 is the remaining identity-vouch question. *Trigger:* the closed service's own post-alpha ruling. **Nothing in the open half depends on the outcome** — templates install through the entitlement seam either way.
