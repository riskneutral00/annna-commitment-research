...
- **The veil derivation, exact** *(2026-08-09; amended 2026-08-22 — the substrate spike proved the shipped values were hand-tuned and fit no function of the stored fields, and the a11y pass proved the one-direction solve inverts on light packs)*. **By mode, one law in two directions** *(a11y A2)*: a **dark** pack solves the veil so a plate composited over the photograph's **brightest** region does not exceed `rgb(92)`; a **light** pack solves so the composite does not **fall below** the value at which the pack's ink clears the engaged AA floor. Then carry **18% of the pack's `DarkMuted`** so it reads as tinted rather than grey. The solve reads **`brightestRegion` — now a stored pack field the admin pipeline measures and writes** *(the V1 fix; `../assets/make-pack.mjs` owes the code half)* — never a per-name table, and **a darker photograph gets a lighter veil**, because it has less to give. `suggestedMode` still selects the skin's ink and which side of §Glass's light/dark density pair applies; it no longer selects the veil. Any future pack derives the same way (`../marketplace/SPEC.md §1.1` points here), **and the install door runs the floors** *(a11y A12)*: a pack whose derived ink, veil or urgency triad misses the AA floors over its own `brightestRegion` and darkest counterpart is refused at install, exactly as one missing a schema field is — the AA promise on installed skins is a gate, not a memo.
  ...
  *(The table above records the finding, never values — the four veils were hand-tuned against these masters and are superseded by the formula, which the fixtures re-derive from once `brightestRegion` is stored.)*
...
- **The scope is an open predicate, not a menu** — a deliberate departure from this corpus's closed-menu discipline, taken by founder ruling. **One cost is owed to the build:** there is no finite scenario suite over scopes — though the **AA proof is finite anyway** *(a11y A14)*: it ranges over `{floor} ∪ stops × installed skins × worst-case regions, and the predicate selects *membership* only. The old second cost (a distinct island `custom` state) **dissolved with the islands** (FD-39): the settings pane's readout *is* the state, and a predicate no preset covers reads back **in words**, never as the string "custom".
- **Keep-awake is what makes an open predicate survivable.** A coarse scope is fine when any single thing can be held out of it.
- **The mark is called `keep-awake`, and the name is load-bearing.** Not *pinned* — the prior build's display flag was `pinned` and collided with its engine's immovable `Placement.kind`, the same collision class that forced the `R#` → `FR#` rename. Not *held* — that is already this corpus's fifth latched mark (`../engine/SPEC.md`, D-B). The mark is **display state only**: never an engine input, never engine truth, stored with the other display-only settings (`SPEC.md §7` — whose drafted FD-42 `display.settings` class is the one seam path that may carry it).
