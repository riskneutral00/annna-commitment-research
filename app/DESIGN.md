...
- Anatomy: **tag chip → title → rule lines → optional meta strip → Confirm / Dismiss**; after confirm it collapses to a ✓ + one-line consequence (`DESIGN:738`). The consequence names the supplied state: accepted into an unsaved draft is not saved or applied (`SPEC.md §3`).
...
- **Pending-decision variant**: a card raised by an engine `PendingDecision` renders its engine-named choices **as the action strip** — the same strip, one action per choice, labelled with the engine's own words. No new anatomy, and no app-authored label (behaviour: `../app/SPEC.md §3`, `../engine/SPEC.md §1.14`). The shared-draft proposal variant instead labels its actions **OK / Edit** and shows the selected object/date or use, field, values and scope; stale actions cannot silently confirm a newer candidate (`../harness/INTERFACES.md §3.1`).

- **Submission controls** (`../harness/INTERFACES.md §3.1`, `SPEC.md §3`): the console input's text-submission action is **Send**; finite supported choices are direct answer controls in the console. Their placement follows this proposal-card variant; the settings-control rejection at §Standing rejections remains. Picker Save and final form Save retain their labels and distinct effects. Keep pending, accepted-in-draft, persisted and refused readbacks distinguishable through words and existing marks, without a new success colour. Removing a proposal's display never claims that its independent stored effects were removed.
...
- **Mobile-first, one column.** The month view, form and template-bound console stack single-column at every width; the compact/tablet thresholds above do not apply — there is nothing to re-arrange. Touch targets keep the 44px minimum. **Body type takes the 16px guest floor** (§Typography) — this is the one read-heavy surface in the product and its reader is a stranger on a phone.
...
- **Conversation is part of the same guest page** (`SPEC.md §5`): keep the request, current fields, proposed terms and explicit confirmation visible in the same plain hierarchy. Distinguish a submitted field answer from confirmation. Refusals and limit/park messages associate with the affected input without losing permitted answers; announce updated proposal/readback under the accessibility law below. Reader/coordinator actions reflect the supplied scope, with no owner settings or general assistant affordances. This is rendering law; the harness/security homes enforce the operation limits.
...
**Added 2026-08-21/22, the visual sitting, same rule** — settings controls inline in the chat body (proposal/answer actions follow §Proposal cards) · free sliders for owner settings · a per-name table for any skin-dependent value · deleting a control instead of relocating it · a resting-dim form.
