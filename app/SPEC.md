...
Which harness instance grounds an interaction (H1 board vs H2 support) is read off what's risen. The app reports the active canvas state with every console utterance so the harness's standing frame carries the right `surface` — the app doesn't interpret, it reports.
