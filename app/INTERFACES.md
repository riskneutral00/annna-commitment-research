...
- **Owns:** rendering every surface; the canvas state machine (board/risers/console — islands retired as chrome, FD-39: their controls live on in the console's settings pane); the catalog manifest and the schema→component mapping; guest token routes; delivery transport + recording; the four app-only views; starter templates and saved-locations surfaces.
