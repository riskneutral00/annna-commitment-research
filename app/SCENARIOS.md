...
- **C5 [surface reported]** An utterance sent with the board risen carries `surface: board`; the same utterance with a commitment riser open carries `surface: commitment-page` + that commitment's ref (values per `../harness/INTERFACES.md` render/act signatures). The app stamps mechanically; no interpretation.
...
- **Z2 [the swap]** The full harness `SCENARIOS.md` suite (including P1, the compaction pass-through) runs green with the real app in place of the app stubs — **zero harness changes**. The layer's definition of done.
