...
- **D3 [failure and complaint recorded]** A bounced/failed send records `delivered-failed` as an event for the harness; a provider-reported spam complaint records `complaint` **against a send that already succeeded** — the app doesn't retry on its own, doesn't stop sending on its own, and decides nothing about what either one means (`SPEC.md §6`; the deciding is `../harness/SPEC.md §3.11`'s).
