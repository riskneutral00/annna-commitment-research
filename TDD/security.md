...
- **N2–N5** — Shop A's rule never evaluates on Shop B's placement (N2); the **engine bind** (`../engine/SPEC.md §7.1`) moves exactly the goal and the counterparty's published windows, by explicit floor-crossed acts, and nothing else (N3); no caller-reachable entry point constructs a two-tenant object (N4); the bind adds no read power beyond the published projection (N5). *(N3 previously named Situation C's referral — referral is deferred and has no seam.)*
