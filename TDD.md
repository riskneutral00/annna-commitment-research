...
- **V3, V4** — every read of a doctor's note logs `{who, basis, when}` (V3); a deletion request walks all four destinations — vault shred + engine crypto-shred + backup age-out + **the firing log's verbatim-member shred** — and produces a completion attestation naming all four (V4).
...
- **R2 [send halt]** — an owner's **bounce rate** past the declared threshold halts that owner's further sends and surfaces the halt *(re-cut 2026-08-31, F-17: the volume-cap half is the deferred row beside R2 at its home — §10 declares a rate, not a cap)*. Per owner, about deliverability; the per-party complaint stop is the harness's (`harness/SCENARIOS.md` D22–D23).
