...
- **Engine stubs** — canned handles and scripted verdicts (the capacity check, the latch check); use the canned `resolve` result roster and share-fixture commit boundaries at `harness/INTERFACES.md §5`.
...
**A** board authoring (incl. **A7** return-leg validation) · **B** rules/elicitation (incl. **B9** the duration floor) · **C** status latches · **D** the floor (incl. **D10–D11** auto-accept-as-Grant, **D18–D20** content-bound confirmation / fail-closed / the floor property, **D22–D23** channel suppression, **D24–D26** the authorization class, **D27** no-signed-term-no-debt, **D28–D29** grant-based reminders and the attributed re-enable) · **D′** the escalation ladder (**D12–D17, D21** — the ladder-walk set re-provenanced `[SHOULD]` under FD-59, D15 the family's `[MUST]`) · **E** M2 gate · **F** conflict/versioning (incl. **F6** in-flight operands) · **G** elicitation store · **H** orders/groups/guest (incl. **H9** the counterparty's move) · **I** the cross-owner share (**I1–I4**) · **K** check-work (**K1–K11** incl. the firing record, the co-write's seam admission, and K11 the ceiling park — C5.0) · **L** context assembly · **N** money · **O** the assisted off-app path (**O1–O5**) · **P** engine-originated round-trips (**P1–P2**) · **R** turn flow (**R1–R7** — dispatch, serialization, the compound sequence, the pre-birth loss record, the registration-kind route) · **X** the external surface (**X1–X8**, X8 the owner's brake) — `[MUST]` throughout save the FD-59 re-provenance above, all behavioral, all on stubs.
...
**Done when.** All MUST scenarios green on stubs. The harness suite then becomes the **reference exam** every other layer must pass at its swap (§Integration).
...
- **D1, D2, D4 [discovery format]** — every published **template** carries exactly one publisher-assigned category from the closed set + tags, with skins a good-level tab, not a category (D1, its home's 2026-08-29 form); the featured shelf shows only admin-flagged items, no computed ranking exists to test around (D2); the catalog carries `popularity` while no v1 surface sorts by it — format presence, UI absence (D4).
...
- **I1, I3, I5** — installing writes a provenance-stamped local copy with no live link or subscription record (I1); a tampered bundle carrying any F1/F2/F3 violation is refused whole, with the failing entry named (I3); uninstalling removes the source document **and any `draft` (disabled) rules the walk left** — every confirmed rule and commitment stands (I5, its home's 2026-08-29 form).
...
- **E2, E3 [degradation]** — service unreachable: everywhere the app offers skins offers the shipped four + Plain **plus any installed pack already persisted on-device**, full function (E2, its home's 2026-08-29 form; drives `app/SCENARIOS.md` S6); every already-installed template stays fully functional, because installed is local forked data (E3).
...
- **V3, V4** — admitted structured medical-answer reads log `{who, basis, when}`; V3's medical-read and consent responsibilities are defined at `security/SCENARIOS.md` V3 and `security/SPEC.md §4`; V4's complete five-obligation deletion walk and completion attestation are defined at `security/SPEC.md §12` and `security/SCENARIOS.md` V4, covering direct vault/engine/firing-log actions, scheduled backup age-out, and provider/BYO retention limits reported as pending or limited rather than as verified destruction.
...
- **R2 [send halt]** — at or above the declared bounce-rate threshold, an owner's further sends halt and the halt surfaces; the owner-scope volume cap is a separate `security/SPEC.md §10` pacing wall whose authorized act is deferred at owner scope and rechecked before a later send, rather than becoming this halt. `security/SCENARIOS.md` R2 asserts the halt; the per-party complaint stop remains distinct in `harness/SCENARIOS.md` D22–D23.
