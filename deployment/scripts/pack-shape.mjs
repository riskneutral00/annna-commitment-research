...
// veil derivation reads at install time, plus the sixteenth (darkestRegion,
// same day's review — the light branch and A12's install floors solve over it). The law had already gone stale against
...
const FIELDS = [
  ...
  "brightestRegion", "darkestRegion", "veil",
];

if (process.argv.includes("--selfcheck")) {
  if (FIELDS.length !== 16) { console.error("selfcheck FAIL: field list is not 16"); process.exit(1); }
...
console.log(`PACK-SHAPE OK — ${packs.length} pack(s), each carrying exactly the ${FIELDS.length} fields marketplace/SPEC.md §1.1 enumerates.
