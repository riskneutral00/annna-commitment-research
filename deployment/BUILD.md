...
- **X1 as an always-run check** — `scripts/x1-secret-grep.mjs`, canary fired above (`../security/BUILD.md` Step 0 owns the scenario).
...
- **R9's noindex half** needs a rung config — none of the three filenames whose one home is `scripts/rung-configs.mjs` (`wrangler.toml`, `wrangler.jsonc`, `wrangler.json`) is present.
