...
> **Partly superseded by the re-scope of 2026-08-08 (FD-4).** The table below is the *record of a review*, not live law, and it is kept whole rather than edited — deleting a finding would erase why a mechanism was ever built. But fourteen scenarios and their mechanisms are gone, so every verdict landing on **B2, B3, B5, B6, B7, Q1, Q2, Q4, S1 or W1–W5** now records a fix to a problem that no longer exists: findings **P1, P2, P6, S1, S4, S5, S9, E2, E3, E5, E6, E8, E10, E16, C6**. Their *reasoning* often survives even where their mechanism does not — P1's deadlock, for one, is answered now by author ≠ approver as discipline (`SPEC.md §4`) rather than by two bot identities. Read the table as history; read `SPEC.md` for what is enforced.
...
| P3 | Per-lie re-pin serializes swap week | **Applied** — batch per swap pass. *Re-pin died with the vendored copy: `SPEC §1` now batches **stub-lies**, not re-pins* | SPEC §1, DR-3 |
...
| S2 | Builder credential scope never specced; injected builder can exfiltrate | **Applied** — credential floor: R6 + repo-scoped tokens. *Branch-limiting went with the branch law (`SPEC §3`): under direct-to-main a token that cannot write main cannot land anything* | SPEC §3/§5, R6 |
...
| S6 | Vendored hash recorded, never verified | ~~**Applied** — merged with E13: content hash recomputed every CI run; provenance checked at re-pin on the floor~~ — **moot: one repo (`SPEC §1`), so there is no vendored copy, no hash and no re-pin** | SPEC §1, S2 |
| S7 | Preview exposure (indexing, debug flags, repo visibility unstated) | **Applied** — noindex + no-debug-flag checks (R9). *(The "code repo declared private" half died with the two-repo split — `SPEC.md §1`; this repo is public, and pushed again since FD-10.)* | SPEC §1/§3, R9 |
...
## Closed by the re-scope: Atomic maturity (raised 2026-08-06, moot 2026-08-08)
...
- ~~The post-launch operations package (monitoring, on-call, incidents) — explicitly out of scope here, unwritten anywhere.~~ **Registered 2026-08-22 (DR-9)**: open with a named readiness condition — `SPEC.md §8`.
