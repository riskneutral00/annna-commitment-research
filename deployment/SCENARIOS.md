...
**Coverage map (SPEC § → scenarios):** two repos & spec import (§1) → S1–S3 · rungs, data/secret/mock/exposure law (§3) → R1–R10 · branch & merge law and the three identities (§4) → B1–B7, Q3 · builder sessions, worktrees, ops lane, toolchain (§5) → W1–W5, B2 (ops-lane exemption) · concurrency, waves ledger, swap lock (§6) → Q1–Q4 · the process floor (§7) → S1 (human-cast re-pin approval), B3 (bypass-free ruleset), B7 (egress allowlist), Z2 (revert reversibility) · merge→deploy (§4) → Z1–Z2, R7. Every `[MUST]` names its mechanism here or in `SPEC.md`; a `[MUST]` enforced only by prose fails its own definition.
