...
| Substrate | Role |
|---|---|
| **GitHub** | This repo · check-runs (no path filters; the always-run `npm run check` aggregator) · **protected environments** (deploy, qualification — the human as required reviewer) · Actions · `gh`. **The deploy environment's required reviewer stands, and nothing here removes it.** How it composes with `SPEC.md §4`'s *"landing on main deploys production"* is **not settled in this table** *(recorded 2026-09-13)*: no deploy workflow exists, so the two statements have never met a mechanism, and who authorizes a production deploy is an authority question owed where the deploy path first becomes readable — `BUILD.md` Step 2. Until then a reader should treat the reviewer as declared and the continuity as ruled, and expect the commit that stands up the first rung to say which governs |
