...
*Status: Phase 1 atoms revised per round-two decisions (H1/M1/M2). The Order object (partial-cancel + group-class) is now **closed** — settled by `../engine/SPEC.md:65` (*"no third mechanism"*). The engine type-value system (M3) remains open — see `06-round-two-decisions.md`.*
