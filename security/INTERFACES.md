...
**Contact readback does not verify a participant.** `../harness/SPEC.md §3.12` owns the declared type/platform, source and optional existing evidence association. Reading or editing it grants no inbox, send or token authority and never mints a party↔principal claim (`SPEC.md §2`). The directory and fire-time read both use the existing per-subject keyed contact path; protected labels/values, previous versions, indexes and derived readback must not leave a plaintext copy that survives key erasure (`SPEC.md §4`). Engine B10 uses the existing keyed fixture, including loss of keys after an earlier owner read; real erasure remains security BUILD Step 3. Logical directory/relationship removal is not erasure or credential revocation.
