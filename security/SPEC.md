...
- **Where the strings live (FD-8, founder-ruled 2026-08-08) — the normative home for the location, and the only one.** The corpus is **`harness/tests/fixtures/injection/`**, committed, carrying a **provenance README** that names each string's public benchmark source. They are public-benchmark-sourced, so committing them publishes nothing that was not already public; what the old "no attack strings in this repo" clause was actually protecting is that annnä does not **author** attack strings inside a public design document, and the provenance file is what answers that. **Fetch-at-test-time is disqualified twice over** — it breaks the hermetic law (`../deployment/SCENARIOS.md` S3) and it breaks replay determinism (B9). Permanent, per the bullet above: any restatement calling this corpus *throwaway* is wrong against this line, not the other way round.
