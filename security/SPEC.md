...
- **Every consent record is version-stamped** — the exact document version consented to, by hash. This composes with document-derived authorization (harness §7 / D8): the *named terms of that version* are the basis for recording its consequences; an unversioned consent is refused at capture, not discovered in a dispute. **And "named terms" are stored structure, never model-read text** *(2026-08-21, drafted-not-ratified — the harness §7 amendment restated from this side)*: the consequences a document names exist as rules with typed operands at signature time, the signature evidences agreement to that ruleset version, and no basis determination ever reads the vault-resident artifact — which the invariant "an artifact never enters the engine store or a seam payload" (§13) already made impossible to do honestly.
...
- **The legal crossings are enumerated — and there is exactly one class at v1: the engine-minted grant** (`../engine/SPEC.md §7.1`, the normative home of the grant vocabulary — founder-ruled 2026-08-07). Data moves across the tenant line only by **explicit granting acts across the floor** — an owner exposing availability, a counterparty accepting a share, an owner widening a viewer — recorded on both boards, never by shared reads (N3).
...
- **The external-client surface is a public door and carries its own named limits** *(2026-08-21 — §13's "every public door has a named limit" was asserting a row this list did not hold)*: seed **60 calls/min per credential**, plus a **per-credential daily model-spend cap** — every inbound call can burn annnä-paid `normalize`/`summarize` (`../model/SPEC.md §7`, no opt-out), so the cap is on annnä's spend, not the caller's. Hitting either **suspends the credential and raises one console card** to the owner; a looping agent becomes one card, never a flooded console and never an invoice. Re-activation is the owner's act, like any withdrawal-and-reissue (§3, fifth row).
- **Email volume is capped** per owner and per recipient — seed caps **200/day per owner, 10/day per recipient**. The floor limits *authorization* of sends (no send without a basis); this limits *volume*; content is already bounded (narrate-only, harness D7). Three different walls, three different failure modes.
...
| Non-owner text cannot instruct the agent | §5 (spotlighting at assembly + dual-model isolation for untrusted channels) |
...
| The engine-minted grant is the only two-tenant edge, and the only crossing class | §9 (N4) |
