...
- **M5 [never both, never neither]** Every mutation entry point is enumerated and each accepts **exactly one** credential model: reachable under an owner session **or** a guest token, never both — and **never neither**, asserted by calling every mutation unauthenticated and observing refusal. M3 tests the "both" half; the "neither" half is what an accidentally public mutation fails (`SPEC.md §2`).
...
**Coverage map (SPEC § → scenarios):** §2 credentials → M3, M5 (never both, never neither), M6 (owner MFA + device-independent recovery) (+ the no-mixing assertions riding T/S fixtures) · §3 tokens → T1–T8 (the fourth class → T7) · §3.1 held credentials (both members) → T8 · §4 vault & classes → V1–V6 · §5 quarantine → Q1–Q4 · §6 consent → S1–S4 · §7 secrets → X1–X5 (the vault key's home → X4; the one-file rule → X5) · §8 DR/exits → D1–D5 (+ V4) · §9 tenants → N1–N5 (the share seam → N3–N5) · §10 abuse → T6, R1–R3 · §11 admin → M1–M4 (hardware-key MFA → M4), V6 · §12 compliance tooling → V4, S3, D2–D4. **Situations:** A → P1–P2 · B → T-family, V1–V2, R-family, S1 · C → V3, Q1, N3, S3–S4. *Flagged gap, carried honestly: no user story exercises an attack — these derive from the Situations as ancestors; a hostile-guest probe story would strengthen the derivations (a future probe, like the marketplace's install-run note).*
