...
- **Y1 [instant resolution — FD-27]** `typed_value` resolves a raw time phrase to an `instant`/`interval` against the board's declared `zone` and a supplied reference instant: *"Thursday 3–4pm"* with zone `Asia/Bangkok` and a Tuesday reference yields that week's Thursday 15:00–16:00 as zone-resolved instants; the identical inputs yield the identical result on replay; a Thai and a zh-TW phrasing of the same time resolve to the same instants; a phrase resolution cannot settle (*"sometime soon"*) returns a `typed_value` **error**, never a guess — and nothing in this path reads wall time. *(The engine half of the thin-agent time law: the model passes the phrase raw, `../model/SPEC.md §5`.)*
