...
- **I2 [both boards or neither]** The counterparty's board fails a check at commit time (a buffer violated by a placement made since the offer): **the commitment appears on neither board**; `{conflict, reason}` returns; the offer goes `stale`.
...
- **O2 [clock trigger, owner decision]** At `start − decide_by` with 2 of 4 seats confirmed, the engine **writes the `needs_human` park and a pending-decision record** and stops there; the harness's trigger loop reads it and the run **parks for a human**. The engine cancels nothing and notifies no customer on its own.
