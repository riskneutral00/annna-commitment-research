...
## I — The cross-owner bind (the handshake, SPEC §7.1)
*Parity family: `../harness/SCENARIOS.md` I1 asserts the harness half (the path resolves as two H1 instances, no separate harness invoked); these assert the engine half.*
- **I1 [engine parity with harness I1]** Two on-app owners, one books the other: `resolve` (over the counterparty's published projection) + `commit(bind_ref)` place **one commitment on each board in one transaction**. **No seam verb beyond the pinned five is called** — asserted as a walk of the contract, which is what makes the swap law survive this feature. *(The engine-side mirror of harness I1 — the two must agree or the swap is a lie.)*
- **I2 [both boards or neither]** The counterparty's board fails a check at commit time (a buffer violated by a placement made since the proposal): **nothing is written on either board**; `{conflict, reason}` returns; the `BindProposal` goes `stale`.
- **I3 [no read power added]** Throughout a bind, every value the initiator's side can observe about the counterparty is derivable from the published Shared projection. No commitment, rule, party, name, or money mark of the counterparty is reachable — asserted as a walk of what the seam returns, not a runtime sample. *(The S1 board-blind guarantee, re-asserted across the tenant line.)*
- **I4 [caller cannot author a crossing]** A caller-supplied structure naming a second tenant is a type error; a `BindProposal` is engine-minted or it does not exist. Asserted structurally against the contract — there is no entry point to call.
- **I5 [silence never binds]** An offered proposal that is never answered reaches `expired` on the virtual clock: no commitment on either board, a recorded terminal event on both.
- **I6 [nothing is held]** While a proposal sits `offered`, its windows remain bookable. Two initiators race for the same counterparty slot: exactly one commits; the loser's alternatives are drawn from the **projection**, never the board.
- **I7 [terminal states recorded both sides]** Decline, expiry, and conflict each write an attributed `{who, basis, when}` event to **both** boards and no commitment to either. *(`../security/SPEC.md §9` N3, made testable.)*
...
- **W5 [template expands to ordinary objects]** A four-day `KindTemplate` (SPEC §1.12) instantiates to four Commitments under one Order, sequenced by `depends_on` per `requires`. **No object type exists in the result that a hand-authored course would not produce** — asserted structurally. *(F7's resolution, made testable.)*
...
- **O2 [clock trigger, owner decision]** At `start − decide_by` with 2 of 4 seats confirmed, the shortfall fires the trigger loop and the run **parks for a human**. The engine cancels nothing and notifies no customer on its own.
...
- **Z2 [the swap]** The full harness `SCENARIOS.md` suite runs green against the real engine with **zero harness changes** — the layer's definition of done. The harness suite **includes P1** (the compaction pass-through), so the proposal path is exercised end-to-end through the harness — Z2 cannot pass vacuously on it.

---

**Coverage map (interview acceptance → scenarios):** no-double-book race → A1 · latch invariant → B1–B3 · diff-only rule writes → A3 · quota math → Q1 · buffer math → T4 · recurrence + single-copy edits → M1–M4 · DST edges → M5–M6 · structural coverage → G4 · unsatisfiable-outranks-authority → G5–G6 · **the class does not over-apply** → G7 · place-only + honest decline → P1–P2 · reshuffle (revisit) → X1–X7 · stub-swap → Z1–Z2 · Situation-D pins → T1/T2/T3, P3/P4, X2, X5 · Situation-C pins → A1 (the last seats), Q2 (the dive pack), P3 (route rotation), B1 (the clocked offer lapses like any hold), P1–P2 + K1–K2 (the week's placements and its money record) · the cross-owner bind → I1–I7 (engine half of harness I1) · multi-day courses → W1–W4, course templates (F7) → W5–W8 · travel envelope → V1–V4 · boat minimum (F20) → O1–O4.
