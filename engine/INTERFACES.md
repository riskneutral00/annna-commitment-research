...
- **A board subscription carries its queried window `{from, to}`** *(2026-08-31 — the member `../app/SCENARIOS.md` C6 gates on, printed seam-side at last)*: a request names its window, bounded exactly as `SPEC.md §9` rules; a window reaching past the loaded horizon **is** §9's beyond-horizon request, and unloaded time is unknown, never free.
