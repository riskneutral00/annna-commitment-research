...
| §1 the closed failure envelope (2026-08-22) — every failure return is one of six kinds, carried as `{kind, reason, detail?, next?}`, reasons from `../harness/COMPAT.md §1`'s table — nothing else | `SPEC.md §6`/`§8`/`§9`'s refusals and declines all surface through the envelope; the engine invents no reason string outside the table *(row added 2026-08-22 — the every-obligation claim again)* |
...
The four projections `SPEC.md §0` enumerates — the guest Shared projection, the owner's live board view, the FR38 template-bundle projection, the FD-34 candidate-shape ghost — are this seam's whole surface; a fifth is a spec change with its own ruling, exactly as a fifth seam verb would be. **Two transports, one fenced family** *(sentence added 2026-08-22 — `SPEC.md §1.7a` and this section previously read as giving the bundle both)*: the guest Shared projection and the owner's live board view are **subscription** members (§2.2's reactive push, the rules below); the template-bundle projection and the candidate-shape ghost are **read-on-demand** members — the bundle invoked as an internal `calculate`-class read (`SPEC.md §1.7a`), the ghost computed when the store surface asks and never stored.
