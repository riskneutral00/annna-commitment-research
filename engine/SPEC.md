...
### §1.12 Kind template — the multi-day shape (F7, resolved 2026-08-06 — **drafted, not ruled**: `../archive/08-founder-rulings-2026-08-06.md` §Provenance)
...
**The one known off-menu request is now on the menu (F20, resolved 2026-08-06 — *drafted, not ruled*; `../archive/08-founder-rulings-2026-08-06.md` §Provenance):** a boat's **minimum passenger count to run** (`../user-stories/Situations/Situation-C/resource-boat.md`) is the `min-occupancy` entry below. *Why it sat off-menu so long is worth printing, because it is the entry's whole design:* every other rule type refuses a write at intake, placement, or commit. `min-occupancy` **cannot** — at the moment of the first booking the count is always below the minimum, so a commit-time gate would make the boat unbookable. It is the menu's only rule that **evaluates at a clock time** and whose violation is **an owner decision, not a refusal**. That is a different shape, which is exactly why improvising it during a build would have been wrong.
...
| `hold` | duration (response window) | kind, offers | commit + clock trigger | sets `expires_at`; lapse fires the trigger loop and the `expired_at` latch (§6.2) |
| `precondition` | precondition-kind list | kind | status derivation | unmet → commitment `pending`; evidence recorded on satisfaction |
...
| **`min-occupancy`** | (unit number `min`, lead duration `decide_by`) | board, kind | **clock trigger** at `start − decide_by` (never at intake or commit) | confirmed concurrent draw at the trigger instant ≥ `min`. **Violation is not a refusal:** the shortfall fires the trigger loop and **parks the run as an owner decision** (run under minimum / cancel / extend the window), because cancelling real people's bookings is an across-the-line act (`../harness/SPEC.md §7`). The engine never auto-cancels a run, and never blocks a booking for being early |
...
**Pairwise clash tests (what `check_consistency` §8 evaluates).** Two rules can clash only where their math meets — overlapping targets and a shared evaluated quantity. The closed menu makes the pair list finite, and this table is exhaustive: same-type pairs never clash (same-level precedence resolves to most-restrictive, above), and a cross-type pair not listed shares no evaluated quantity — structurally clash-free, their checks compose. §8 routes each hit to its class: rows marked ***unsatisfiable*** are **refused at write whatever the authority** (no override can make them satisfiable — §8 item 4); the three rows marked *latent* always classify latent; everything else routes by authority level (governing conflict / own-rule conflict).
...
**The v1 bound, printed — it bounds the *reshuffle* search:** single board-owner, single day, **at most 3 moves per proposal**. *(It is a bound on rearranging existing commitments. It is not a claim that `resolve` only ever sees one owner's boards — the §7.1 bind reads a second owner's **published projection** and moves nothing, so it is outside this bound by construction, not by exception.)* The promise is *feasibility and improvement in the chosen direction* — not optimality. (Week-level batching and multi-owner reshuffles are §11 non-goals; this bound is what keeps `resolve` a checker, not a routing optimizer.)
...
### §7.1 The handshake — the cross-owner bind seam

*The mechanism cited by `../harness/SPEC.md §2` ("two H1 instances reconciled by the engine handshake"), `../harness/SCENARIOS.md` I1, `../harness/BUILD.md` Step 5, and `../security/SPEC.md §9` (the one enumerated legal crossing at v1). Defined here, once.*

> **Provenance: drafted, not ruled.** This section was authored on the founder's behalf during the 2026-08-06 repair pass and has not been separately ratified — see `../archive/08-founder-rulings-2026-08-06.md` §Provenance. It is the largest such decision in the corpus. Treat its reasoning as a proposal and its presence here as provisional; where it disagrees with a founder ruling, the ruling wins.

**What it is, in one line:** when both parties are on-app, a booking is **one commitment placed into two owners' boards in one transaction** — not a message, not a request queue, not a shared read.

**It is NOT the §11 reshuffle handshake.** This seam **places new commitments into already-published free space**; it never moves, re-times, or re-shapes a commitment that already exists on either board. The banned thing at §11 is a *multi-owner reshuffle* — an optimizer that rearranges two people's existing days to make room. That stays banned, and the §7 v1 bound (single owner, single day, ≤3 moves) is unchanged: **reshuffle is single-owner; bind is two-owner; they are different operations.** A reader who conflates them re-derives a conflict that is not there.

**No new read power — the projection is the whole window.** The initiator never queries the counterparty's board. It sees exactly what the counterparty published: the **Shared projection** (§1.7), the same object a guest link exposes, with §1.7's leak guarantees intact. *Reachability is not readability.* This is why the bind adds no tenant-isolation surface: nothing readable becomes readable.

**The one legal cross-tenant object.** `../security/SPEC.md §9` makes a cross-tenant reference *unconstructable*, and that stands: no caller may author a field naming another tenant. The bind instead runs through a **`BindProposal`** — engine-minted, engine-owned, resident in **neither** tenant:

```
BindProposal {
  id
  initiator      -> { tenant, board, principal }
  counterparty   -> { tenant, board }        // derived from the projection, never caller-authored
  goal                                        // the commitment to be placed
  offered          -> Handle                  // windows read from the counterparty's projection
  status           : offered | accepted | declined | expired | applied | stale
  expires
}
```

It is the only object in the store permitted to name two tenants, and **no caller can construct one** — the engine mints it or nothing does. That keeps the invariant literal ("unconstructable by a caller") while making the crossing possible.

**The seam, exactly — and it is not a new one.** The bind adds **no verb** to the engine's surface. The harness contract pins five obligations (`../harness/INTERFACES.md §1`) and the swap law is *zero harness changes*; a sixth verb would break both. The bind rides the two that already exist:

- **`resolve(goal, boards, rules) → Handle`** — when `boards` includes a counterparty's **published projection ref** rather than a board the caller owns, `resolve` validates the goal against the published windows and returns a **`BindProposal` handle**. This is the seam's stated shape already: *"a placement, a proposal, or a structured decline — same seam signature."* A bind is the third kind of proposal, alongside the compaction Proposal (§1.11). It writes nothing.
- **`commit(bind_ref)`** — the existing **apply-proposal variant** (§6.5): takes a proposal handle, re-runs every check as of *now*, applies atomically or returns `{conflict}`. A bind proposal is applied by the same call, under the same law, with one addition printed below: the counterparty's basis must be present.

*Why this is worth stating out loud:* the temptation is to give a cross-tenant operation its own dedicated entry point, and that instinct is exactly wrong here. A second write path is a second place for the tenant line to leak, a second thing the stub must fake, and a harness change at swap time. One seam, one set of checks.

**The five questions, answered:**

1. **What crosses.** Only the goal and the published windows. Never the counterparty's other commitments, rules, parties, or money marks — none are readable to construct with.
2. **Consent is the counterparty's floor act, not the engine's.** `status: offered` fires the **counterparty's** trigger loop (`../harness/SPEC.md §4`). Accepting places a real obligation on a real person, so it is across the line **on their side**: it needs a live confirmation or a matching stored Grant (`../harness/SPEC.md §7` — the setup-time auto-accept Grant defined there is exactly this basis). **Silence is not acceptance** — an un-answered proposal expires, it never binds.
3. **What commits.** `commit(bind_ref)` re-runs **every** §6.1 check on **both** boards as of *now* — capacity, buffers (including travel), latches, preconditions, governing rules — inside **one atomic transaction**, and writes the paired commitments together. **Both boards or neither.** A partial bind cannot exist.
4. **Contention.** A `BindProposal` **holds nothing** — offered windows stay bookable until commit. Two initiators racing for one counterparty slot: at most one commits; the loser gets `{conflict, reason, alternatives}` where the alternatives come from the **projection**, never the board. The proposal goes `stale`; re-proposing is a fresh read, per §4's staleness law.
5. **Partial failure and the honest dead end.** Decline, expiry, and commit-time conflict are all first-class terminal states that write **no** commitment on either side and return a structured reason. Each terminal state is recorded on **both** boards as an attributed event `{who, basis, when}` — which is what `../security/SPEC.md §9`'s N3 ("recorded on both boards, never by shared reads") means in practice.

**Off-app counterparties are unaffected.** The form-export path (`../harness/SPEC.md §2`) is untouched: no agent, no tenant, no BindProposal. The handshake is the *both-on-app* case only.

**Referral is a different thing and is deferred.** The handshake reaches a counterparty **the initiator already holds a projection for**. Handing a customer to another centre — discovering a stranger, passing their data — is the referral flow, deferred (see `../README.md`). The handshake does not build toward it and must not be read as a partial version of it.
...
- `check_consistency(rule | ruleset) → {conflicts, latent}` classifies **four** ways. The first three route by **authority**; the fourth routes by **satisfiability** and outranks them.
  ...
  Because the menu is closed, pairwise conflict conditions are **enumerable** — the spec of each pair's clash test lives with the menu entries (§3), not in open-ended logic, and §3's table marks which rows are unsatisfiable.
...
| A bind writes both boards or neither; no partial cross-owner state | §7.1 |
| Only the engine mints a two-tenant reference; no caller can author one | §7.1 |
