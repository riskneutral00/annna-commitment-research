...
|---|---|---|---|---|
| `buffer` | constant duration, **or** computed travel | board, kind, party | placement (calculate, resolve, commit) | gap between adjacent placements ≥ operand; the computed form is `travel(a.place, b.place, at)` per §5 precedence — **one type, two operand forms** (harness `Rule{type: buffer, operand: 5min}` stores unchanged; the travel-gap is its computed case). **The gap requirement a committed placement carries binds at its own commit** *(2026-08-21, drafted-not-ratified — Situation-B's "units already cooling down finish the six they entered with")*: an operand edit governs placements committed after it; a window already running off an existing placement neither shortens nor lengthens (harness F6). Violation: placement infeasible → not offered / `{conflict}` |
...
| `pricing` | money (+ optional per-unit rate) | kind, audience | **binds at booking**; marked at settle | booked commitments keep their price forever; edits govern forward only. Violation result: none exists — a bound price is a term, not a gate; nothing evaluates against it to refuse (stated because §3's gate demands every row say so) |
